...
**Flutter 3.x · Riverpod 2.x · Drift v7 · Azure SQL · RevenueCat · Multi-Model AI**

> **Offline-First · Privacy-Centric · Built for India · AI-Adaptive**
> Dark mode primary · Glassmorphism · Spring physics · Bento grid
> All backend via **Azure** (Azure SQL + Azure Functions) — infrastructure as code.
...
| **Cloud architecture** | 8 Azure Functions, **fan-out orchestration** for the daily batch job | Hardened in v1.0 — see §P0-C and §AZ |
...
2. **Daily Intelligence Package generation** ran as a sequential per-user loop inside a single timer-triggered Azure Function, which would time out or fail as an all-or-nothing batch past a few hundred users. Replaced with a Durable Functions fan-out/fan-in orchestration with per-user error isolation. See §P0-C and §AZ.
...
| Azure SQL setup | §AZ Azure Setup |
...
Health OS Brain ────────────────────────────────────────┐
    ...
    ↓ (background sync)
Sync Worker → Azure SQL Database
    ↓ (event-triggered only)
AI Router → Appropriate Model Tier → Azure Function
```
...
| **Sync Engine** | Priority queue + DLQ | Background writes to Azure SQL via REST; retries 3× then DLQ |
| **Cloud** | Azure (SQL + Functions + Entra B2C) | Cross-device sync, AI functions, auth |
...
**Azure:**
- Azure subscription (student tier sufficient for dev)
- Azure CLI + Functions Core Tools v4
- Node.js 20.x for Azure Functions
...
class AiCoachChatNotifier extends _$AiCoachChatNotifier {
  ...
  Future<void> sendMessage({
    ...
    try {
      // 2. Fetch response from Azure Function
      final response = await _callAzureCoachFunction(text, conversationId);
  ...
  Future<AzureCoachResponse> _callAzureCoachFunction(String message, String convId) async {
    // API call wrapper...
    return AzureCoachResponse(reply: "Mocked AI reply based on data", sources: ["7-day logs"]);
  }
}
...
### Azure Function: `fitkarma-coach` (v1)
...
class RestaurantDatabaseService {
  ...
  final AzureClient _azureClient;

  RestaurantDatabaseService(this._localDb, this._azureClient);
  ...
  Future<List<RestaurantMenuItem>> search({
    ...
    // 2. Fall back to Azure SQL restaurant DB
    return await _azureClient.searchRestaurantMenu(restaurantName, city);
  }
...
class KarmaHubNotifier extends StateNotifier<KarmaHubState> {
  ...
  /// Optimistically awards XP locally before syncing to Azure
  Future<void> awardXp({
    required int amount,
...
class CohortInsightsService {
  ...
  Future<CohortInsights> getInsights(String userId) async {
    ...
    // Call Azure Endpoint to fetch aggregated regional/demographic cohort distribution data
...
- **Optimistic Interactions**: High-fives use Drift database writes to instantly render spring-loaded icon reactions before syncing to the Azure database.
...
- Only extracted numeric values + report type sync to Azure SQL
...
- **Local-First Isolation**: Medication schedules, taken timestamps, and uploaded lab report PDFs are encrypted locally using AES-256 (via SQLCipher) and are never synced to Azure unless the user explicitly opts in for cloud backups.
- **Consent Revocability**: In compliance with the India Digital Personal Data Protection (DPDP) Act, users have a single-tap "Revoke All Clinical Access" setting. This immediately wipes all CGM readings, medication logs, and doctor share tokens from both the local SQLite/Drift store and Azure SQL.
...
| AI context | User context never logged in Azure Function logs |
    ...
    steps:
      - run: |
          flutter build appbundle --release \
            --dart-define=AZURE_SQL_ENDPOINT=${{ secrets.AZURE_SQL_ENDPOINT }} \
            --dart-define=AZURE_FUNCTION_BASE_URL=${{ secrets.AZURE_FUNCTION_BASE_URL }} \
            --dart-define=AZURE_ENTRA_CLIENT_ID=${{ secrets.AZURE_ENTRA_CLIENT_ID }} \
...
WhatsApp Business Cloud API webhook
    ↓
Azure Function: fitkarma-whatsapp (NEW)
    ↓
Resolve phone number → userId (via linked azureId)
    ↓
...
```javascript
// Azure Function: fitkarma-whatsapp (HTTP Trigger — webhook)
app.http('fitkarma-whatsapp', {
  methods: ['POST'],
  handler: async (request, context) => {
    const payload = await request.json();
    const message = payload.entry?.[0]?.changes?.[0]?.value?.messages?.[0];
    if (!message) return { status: 200 }; // ignore non-message webhook events

    const phoneNumber = message.from;
    const user = await resolveUserByPhone(phoneNumber);
    ...
    if (message.type === 'text') {
      logResult = await parseFoodTextAndLog(user.userId, message.text.body);
    } else if (message.type === 'image') {
      const imageBuffer = await downloadWhatsAppMedia(message.image.id);
      logResult = await analyzeMealPhotoAndLog(user.userId, imageBuffer); // reuses §P5-C pipeline
    } else {
...
- Phone number is stored only as a lookup key linked to the existing `azureId`, encrypted at rest in Azure SQL like other PII.
...
User taps mic → speaks in their language
    ↓
On-device or cloud ASR (Azure Speech-to-Text, multi-language)
    ↓
...
class VoiceLogService {
  final SpeechToTextClient _asrClient; // Azure Speech SDK wrapper
  ...
  Future<LogResult> logFromVoice({
    ...
    final transcript = await _asrClient.transcribe(
      audio: audioBytes,
      languageCode: _toAzureLocale(preferredLanguage), // e.g. 'hi-IN', 'ta-IN'
    );
  ...
  String _toAzureLocale(String appLanguageCode) {
    const localeMap = {
...
## §DB-C. Azure SQL Cloud Database Schema

To support offline-first sync consistency, the Azure SQL Database serverless instance mirrors the local Drift SQLite schemas. In addition, it implements tables for sync orchestration, telemetry, and conflict resolution auditing.
...
CREATE TABLE [dbo].[users] (
    ...
    [workStyle] NVARCHAR(50) NOT NULL,
    [goals] NVARCHAR(MAX) NOT NULL, -- JSON array
    [dosha] NVARCHAR(50) NULL,
    [currentProgram] NVARCHAR(100) NULL,
    [dietType] NVARCHAR(50) NOT NULL,
    [region] NVARCHAR(100) NULL,
    [tdee] FLOAT NOT NULL,
    [dailyStepsTarget] INT NOT NULL,
    [dailyCalorieTarget] INT NOT NULL,
    [dailyWaterTargetL] FLOAT NOT NULL,
    [dailyProteinTargetG] INT NOT NULL,
    [tone] NVARCHAR(50) NOT NULL DEFAULT 'motivational',
    ...
    [familyUnitId] NVARCHAR(128) NULL,
    [averageReliabilityPct] FLOAT NOT NULL DEFAULT 100.0,
    [athleticProfileJson] NVARCHAR(MAX) NOT NULL,
    [athleticTestBatteryJson] NVARCHAR(MAX) NOT NULL,
    [skillMasteryLevelsJson] NVARCHAR(MAX) NOT NULL,
    [projectedPerformanceJson] NVARCHAR(MAX) NOT NULL,
    -- v1.0 (v17): timezoneOffsetMinutes/preferredDIPHour replace the hardcoded
    -- 6am IST batch schedule; see fitkarma-health-os in §AZ.
    ...
    [abhaHealthId] NVARCHAR(64) NULL, -- encrypted at rest; see §P16-C
...
CREATE NONCLUSTERED INDEX [IX_user_scores_lookup]
  ON [dbo].[user_scores] ([userId], [scoreType], [computedAt] DESC);
```
...
CREATE TABLE [dbo].[organization_accounts] (
    ...
    [accountType] NVARCHAR(20) NOT NULL,   -- 'employer' | 'insurer'
...
CREATE TABLE [dbo].[food_logs] (
    ...
    [fat] FLOAT NOT NULL,
    [processingTier] FLOAT NOT NULL DEFAULT 1.0,
    [hasGlycemicAnalysis] BIT NOT NULL DEFAULT 0,
    [createdAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_food_logs] PRIMARY KEY CLUSTERED ([localId] ASC),
    CONSTRAINT [FK_food_logs_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId]) ON DELETE CASCADE
);

CREATE NONCLUSTERED INDEX [IX_food_logs_userId_consumeTime] ON [dbo].[food_logs] ([userId] ASC, [consumeTime] DESC);
```

#### Daily Intelligence Packages Table
```sql
CREATE TABLE [dbo].[daily_intelligence_packages] (
    [localId] NVARCHAR(128) NOT NULL,
    [userId] NVARCHAR(128) NOT NULL,
    [packageDate] DATETIME2 NOT NULL,
    [primaryInsight] NVARCHAR(MAX) NOT NULL,
    [todaysMission] NVARCHAR(MAX) NOT NULL,
    [nutritionFocus] NVARCHAR(MAX) NOT NULL,
    [recoveryFocus] NVARCHAR(MAX) NOT NULL,
    [motivationMessage] NVARCHAR(MAX) NOT NULL,
    [adjustedCalories] INT NOT NULL,
    [adjustedProtein] INT NOT NULL,
    [adjustedHydrationL] FLOAT NOT NULL,
    [recommendedIntensity] NVARCHAR(50) NOT NULL,
    [isRestDay] BIT NOT NULL DEFAULT 0,
    [activeRisks] NVARCHAR(MAX) NOT NULL, -- JSON array
    [showFestivalBanner] BIT NOT NULL DEFAULT 0,
    [festivalAdaptation] NVARCHAR(MAX) NULL,
    [dietBreakActive] BIT NOT NULL DEFAULT 0,
    [proteinTimingTarget] INT NOT NULL DEFAULT 25,
    [loggingReliabilityStatus] NVARCHAR(50) NOT NULL DEFAULT 'high',
    [satietyTargetScore] INT NOT NULL DEFAULT 70,
    [aiCallsUsed] INT NOT NULL DEFAULT 0,
    [createdAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_daily_intelligence_packages] PRIMARY KEY CLUSTERED ([localId] ASC),
    CONSTRAINT [FK_daily_intelligence_packages_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId]) ON DELETE CASCADE
);

CREATE NONCLUSTERED INDEX [IX_dip_userId_date] ON [dbo].[daily_intelligence_packages] ([userId] ASC, [packageDate] DESC);
```

#### CGM Readings Table
```sql
CREATE TABLE [dbo].[cgm_readings] (
    [readingId] NVARCHAR(128) NOT NULL,
    [userId] NVARCHAR(128) NOT NULL,
    [timestamp] DATETIME2 NOT NULL,
    [glucoseMgDl] FLOAT NOT NULL,
    [trend] NVARCHAR(50) NOT NULL,
    [status] NVARCHAR(50) NOT NULL,
    CONSTRAINT [PK_cgm_readings] PRIMARY KEY CLUSTERED ([readingId] ASC),
    CONSTRAINT [FK_cgm_readings_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId]) ON DELETE CASCADE
);

CREATE NONCLUSTERED INDEX [IX_cgm_readings_userId_timestamp] ON [dbo].[cgm_readings] ([userId] ASC, [timestamp] DESC);
```

#### Recovery Logs Table
```sql
CREATE TABLE [dbo].[recovery_logs] (
    [localId] NVARCHAR(128) NOT NULL,
    [userId] NVARCHAR(128) NOT NULL,
    [logDate] DATETIME2 NOT NULL,
    [readinessScore] INT NOT NULL,
    [confidenceTier] NVARCHAR(50) NOT NULL DEFAULT 'basic',
    [sleepQuality] INT NOT NULL,
    [sorenessLevel] INT NOT NULL,
    [stressLevel] INT NOT NULL,
    [energyLevel] INT NOT NULL,
    [restingHR] FLOAT NULL,
    [hrv] FLOAT NULL,
    [sorenessRegions] NVARCHAR(MAX) NOT NULL,
    [sleepNeedMinutes] INT NOT NULL DEFAULT 480,
    [sleepPerformanceScore] INT NOT NULL DEFAULT 100,
    [dailyStrainScore] FLOAT NOT NULL DEFAULT 0.0,
    [illnessRiskStatus] NVARCHAR(50) NOT NULL DEFAULT 'low',
    [prescribedActionsJson] NVARCHAR(MAX) NOT NULL,
    [recoveryDriversJson] NVARCHAR(MAX) NOT NULL,
    [createdAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_recovery_logs] PRIMARY KEY CLUSTERED ([localId] ASC),
    CONSTRAINT [FK_recovery_logs_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId]) ON DELETE CASCADE
);
```

#### Transformation Checks Table
```sql
CREATE TABLE [dbo].[transformation_checks] (
    [localId] NVARCHAR(128) NOT NULL,
    [userId] NVARCHAR(128) NOT NULL,
    ...
    [measurementsJson] NVARCHAR(MAX) NULL,
    [createdAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_transformation_checks] PRIMARY KEY CLUSTERED ([localId] ASC),
    CONSTRAINT [FK_transformation_checks_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId]) ON DELETE CASCADE
);
```
...
```sql
CREATE TABLE [dbo].[sync_audit_trail] (
    [auditId] UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(),
    [userId] NVARCHAR(128) NOT NULL,
    [syncDirection] NVARCHAR(10) NOT NULL, -- 'PUSH' or 'PULL'
    [tableName] NVARCHAR(128) NOT NULL,
    [recordsProcessed] INT NOT NULL,
    [syncStatus] NVARCHAR(20) NOT NULL, -- 'SUCCESS', 'FAILED', 'PARTIAL'
    [errorMessage] NVARCHAR(MAX) NULL,
    [durationMs] INT NOT NULL,
    [processedAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_sync_audit_trail] PRIMARY KEY CLUSTERED ([auditId] ASC),
    CONSTRAINT [FK_sync_audit_trail_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId])
);

CREATE NONCLUSTERED INDEX [IX_sync_audit_trail_userId] ON [dbo].[sync_audit_trail] ([userId] ASC);
```
...
```sql
CREATE TABLE [dbo].[sync_dead_letter_queue] (
    [dlqId] UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(),
    [userId] NVARCHAR(128) NOT NULL,
    [tableName] NVARCHAR(128) NOT NULL,
    [recordLocalId] NVARCHAR(128) NOT NULL,
    [payloadJson] NVARCHAR(MAX) NOT NULL,
    [failureReason] NVARCHAR(MAX) NOT NULL,
    [retryCount] INT NOT NULL DEFAULT 0,
    [status] NVARCHAR(20) NOT NULL DEFAULT 'PENDING', -- 'PENDING', 'RETRIED', 'ABANDONED'
    [queuedAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    [lastAttemptAt] DATETIME2 NULL,
    CONSTRAINT [PK_sync_dead_letter_queue] PRIMARY KEY CLUSTERED ([dlqId] ASC),
    CONSTRAINT [FK_sync_dlq_users] FOREIGN KEY ([userId]) REFERENCES [dbo].[users] ([localId])
);

CREATE NONCLUSTERED INDEX [IX_sync_dlq_status] ON [dbo].[sync_dead_letter_queue] ([status] ASC);
```
...
```sql
CREATE TABLE [dbo].[marketplace_wallets] (
    ...
    [createdAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    CONSTRAINT [PK_marketplace_wallets] PRIMARY KEY CLUSTERED ([walletId] ASC),
    CONSTRAINT [FK_marketplace_wallets_users] FOREIGN KEY ([creatorId]) REFERENCES [dbo].[users] ([localId])
);

CREATE TABLE [dbo].[marketplace_ledger_entries] (
    [entryId] NVARCHAR(128) NOT NULL,
    [transactionId] NVARCHAR(128) NOT NULL,
    [walletId] UNIQUEIDENTIFIER NULL,
    [accountType] NVARCHAR(30) NOT NULL, -- 'escrowLiability', 'creatorPayable', 'platformRevenue', 'tcsLiability', 'tdsLiability', 'gstExpense', 'cashAsset'
    [debit] DECIMAL(18, 4) NOT NULL DEFAULT 0.0000,
    [credit] DECIMAL(18, 4) NOT NULL DEFAULT 0.0000,
    [timestamp] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
    [memo] NVARCHAR(MAX) NULL,
    CONSTRAINT [PK_marketplace_ledger_entries] PRIMARY KEY CLUSTERED ([entryId] ASC),
    CONSTRAINT [FK_marketplace_ledger_wallets] FOREIGN KEY ([walletId]) REFERENCES [dbo].[marketplace_wallets] ([walletId])
);

CREATE NONCLUSTERED INDEX [IX_ledger_entries_wallet] ON [dbo].[marketplace_ledger_entries] ([walletId] ASC);
CREATE NONCLUSTERED INDEX [IX_ledger_entries_tx] ON [dbo].[marketplace_ledger_entries] ([transactionId] ASC);
```

---


# §AZ. Azure Functions — All Functions
...
```json
{
  ...
  "Values": {
    ...
    "SQL_CONNECTION_STRING": "Server=tcp:fitkarma-sql-server.database.windows.net,...",
...
```javascript
// Runs every 15 minutes. Fans out DIP generation to users whose
// local time currently matches their preferred generation hour,
// instead of assuming every user is in IST.

const df = require('durable-functions');

app.timer('fitkarma-health-os-trigger', {
  schedule: '0 */15 * * * *', // every 15 minutes
  handler: async (myTimer, context) => {
    const client = df.getClient(context);
    await client.startNew('healthOSOrchestrator', undefined, {});
  }
});

// Orchestrator: fans out one activity invocation per eligible user,
// runs them in parallel, and isolates failures per user.
df.app.orchestration('healthOSOrchestrator', function* (context) {
  const eligibleUsers = yield context.df.callActivity('getUsersDueForDIP');
  // getUsersDueForDIP filters activeUsers by whether the user's
  // local time (UTC now + Users.timezoneOffsetMinutes) falls within
  // the current 15-minute window of their preferredDIPHour.

  const tasks = eligibleUsers.map(userId =>
    context.df.callActivity('generateDIPForUser', userId)
  );

  // Task.all resolves when all activities settle; a rejected activity
  // does not block or fail the others — errors are caught per-activity
  // below and logged individually rather than aborting the batch.
  const results = yield context.df.Task.all(
    tasks.map(t => t.catch(err => ({ error: err.message })))
  );

  return results;
});

// Activity function: does the actual per-user work in isolation.
df.app.activity('generateDIPForUser', {
  handler: async (userId, context) => {
    try {
      // Step 1: Compute health snapshot (no AI)
      const snapshot = await computeHealthSnapshot(userId);

      // Step 2: Check if AI is even needed
      const trigger = checkAITrigger(snapshot);
      if (!trigger && hasCachedDIP(userId)) {
        // Reuse yesterday's DIP with today's computed targets
        await refreshDIPTargets(userId, snapshot);
        return { userId, status: 'reused' };
      }

      // Step 3: Single AI call — generate DIP
      const dip = await generateDIP(userId, snapshot);

      // Step 4: Store in Azure SQL + queue sync to user's Drift
      await storeDIP(userId, dip);
      await queueDriftSync(userId, dip);
      return { userId, status: 'generated' };
    } catch (err) {
      // Per-user failure is logged and does not propagate to the
      // orchestrator's other parallel tasks — see Task.all above.
      context.error(`DIP generation failed for ${userId}: ${err.message}`);
      throw err;
    }
  }
});

async function generateDIP(userId, snapshot) {
...
async function getCached(pool, userId, promptHash) {
  const result = await pool.request()
    .input('userId', sql.NVarChar, userId)
    .input('hash', sql.NVarChar, promptHash)
    .query(`
      SELECT response FROM ai_cache
      WHERE user_id = @userId AND prompt_hash = @hash AND expires_at > GETUTCDATE()
    `);
  return result.recordset[0]?.response ?? null;
}

async function setCached(pool, userId, promptHash, response, ttlHours = 24) {
  await pool.request()
    .input('userId', sql.NVarChar, userId)
    .input('hash', sql.NVarChar, promptHash)
    .input('response', sql.NVarChar(sql.MAX), response)
    .input('expires', sql.DateTime2, new Date(Date.now() + ttlHours * 3600000))
    .query(`
      MERGE ai_cache AS target
      USING (SELECT @userId AS user_id, @hash AS prompt_hash) AS source
        ON target.user_id = source.user_id AND target.prompt_hash = source.prompt_hash
      WHEN MATCHED THEN UPDATE SET response = @response, expires_at = @expires
      WHEN NOT MATCHED THEN INSERT (user_id, prompt_hash, response, expires_at)
        VALUES (@userId, @hash, @response, @expires);
    `);
}
...
async function purgeCacheForUser(pool, userId) {
  await pool.request()
    .input('userId', sql.NVarChar, userId)
    .query('DELETE FROM ai_cache WHERE user_id = @userId');
}
```
...
app.http('fitkarma-social', {
  methods: ['POST', 'GET'],
  authLevel: 'anonymous', // Secured via Entra JWT validation middleware
  handler: async (request, context) => {
    const pool = await sql.connect(process.env.SQL_CONNECTION_STRING);
    const userId = request.headers.get('x-user-id'); // Set by API Gateway after token verification
    ...
    if (request.method === 'POST') {
      ...
      if (action === 'high-five') {
        ...
        // 1. Transaction to register reaction and award poster XP
        const transaction = new sql.Transaction(pool);
        await transaction.begin();
        try {
          // Increment reaction count
          await transaction.request()
            .input('itemId', sql.VarChar, targetFeedItemId)
            .input('userId', sql.VarChar, userId)
            .query(`
              INSERT INTO feed_reactions (item_id, user_id, reaction_type)
              VALUES (@itemId, @userId, 'high_five');
            `);
          ...
          await transaction.request()
            ...
            .query(`
              ...
              WHERE localId = (SELECT user_id FROM feed_items WHERE item_id = @itemId);
            `);
      ...
      if (action === 'post-activity') {
        ...
        await pool.request()
          ...
          .query(`
            ...
            VALUES (@userId, @type, @payload, @privacy, GETUTCDATE());
          `);
    ...
    if (request.method === 'GET' && action === 'leaderboard') {
      ...
      const result = await pool.request()
        ...
        .query(`
          SELECT TOP 50 name, health_score, steps_7d_avg
          FROM users
          WHERE region = @city AND leaderboard_opt_in = 1
          ORDER BY health_score DESC;
        `);
      return { status: 200, body: JSON.stringify(result.recordset) };
    }

    return { status: 400, body: 'Invalid action or method.' };
  }
});
```
...
app.http('fitkarma-marketplace', {
  ...
  handler: async (request, context) => {
    ...
    if (request.method === 'GET' && action === 'match') {
      // Ingests client demographics and goals to match with relevant coaches
      const userResult = await pool.request()
        .input('userId', sql.VarChar, userId)
        .query('SELECT goals, dietType FROM users WHERE localId = @userId');
      
      const user = userResult.recordset[0];
      const coachesResult = await pool.request()
        .query('SELECT creatorId, name, specialties, rateInr, rating FROM creator_profiles');
      
      return { status: 200, body: JSON.stringify({ user, coaches: coachesResult.recordset }) };
    }

    if (request.method === 'POST' && action === 'hire') {
      ...
      await pool.request()
        .input('userId', sql.VarChar, userId)
        .input('coachId', sql.VarChar, coachId)
        .query(`
          INSERT INTO coach_clients (coach_id, client_id, active_from, has_write_permission)
          VALUES (@coachId, @userId, GETUTCDATE(), 1);
        `);
      
      return { status: 200, body: JSON.stringify({ success: true, message: 'Coach successfully hired.' }) };
    }

    if (request.method === 'POST' && action === 'payout') {
      // Calculate and trigger payout to creator wallet
      const payoutResult = await pool.request()
        .input('userId', sql.VarChar, userId)
        .query(`
          SELECT SUM(amount_inr * 0.8) as earnings
          FROM creator_transactions
          WHERE creator_id = @userId AND payout_status = 'pending';
        `);
      
      const totalEarnings = payoutResult.recordset[0]?.earnings ?? 0;
      // Integration with payment gateway (Razorpay / Stripe) to transfer balance...
      return { status: 200, body: JSON.stringify({ processedEarnings: totalEarnings }) };
    }
...
const { app } = require('@azure/functions');
const sql = require('mssql');
const { getCached, setCached, hashPrompt } = require('./shared/aiCache');

app.http('fitkarma-cores', {
  methods: ['POST'],
  authLevel: 'anonymous',
  handler: async (request, context) => {
    const pool = await sql.connect(process.env.SQL_CONNECTION_STRING);
    ...
    if (action === 'generate-diet') {
      ...
      // Check Cache first to save token cost
      const cachedResponse = await getCached(pool, promptHash);
      if (cachedResponse) {
      ...
      await setCached(pool, promptHash, dietResponse.content, 168); // Cache diet plan for 7 days
      return { status: 200, body: dietResponse.content };
    }
    ...
    if (action === 'generate-blueprint') {
      ...
      await setCached(pool, promptHash, blueprintResponse.content, 720); // Cache program blueprint for 30 days
      return { status: 200, body: blueprintResponse.content };
    }
...
app.http('fitkarma-coach', {
  ...
  handler: async (request, context) => {
    ...
    // 1. Fetch user health snapshot & long-term memory
    const userResult = await pool.request()
      .input('userId', sql.VarChar, userId)
      .query(`
        SELECT health_score, primary_personality, system_tone 
        FROM users WHERE localId = @userId;
        SELECT TOP 10 role, content FROM chat_history 
        WHERE session_id = @chatSessionId ORDER BY created_at DESC;
      `);

    const user = userResult.recordsets[0][0];
    const rawHistory = userResult.recordsets[1].reverse();
    ...
    const metricsResult = await pool.request()
      .input('userId', sql.VarChar, userId)
      .query('SELECT TOP 7 date, calories, protein, completed FROM daily_logs WHERE user_id = @userId ORDER BY date DESC');
    ...
    const systemPrompt = `
      ...
      Recent 7-day logs: ${JSON.stringify(metricsResult.recordset)}.
    ...
    // 6. Save message pair to history database
    await pool.request()
      .input('sessionId', sql.VarChar, chatSessionId)
      .input('userMsg', sql.NVarChar, message)
      .input('botMsg', sql.NVarChar, botMessage)
      .query(`
        INSERT INTO chat_history (session_id, role, content, created_at)
        VALUES (@sessionId, 'user', @userMsg, GETUTCDATE()),
               (@sessionId, 'assistant', @botMsg, GETUTCDATE());
      `);

    return { status: 200, body: JSON.stringify({ reply: botMessage }) };
  }
});
```
...
app.http('fitkarma-meal-vision', {
  ...
  handler: async (request, context) => {
    ...
    const imgHash = crypto.createHash('sha256').update(imageBase64).digest('hex').slice(0, 16);
    
    // Check if this exact image was analyzed recently
    const cachedResponse = await getCached(pool, imgHash);
    if (cachedResponse) {
      return { status: 200, body: cachedResponse };
    }
    ...
    await setCached(pool, imgHash, visionResponse.content, 72); // Cache image hash for 3 days
    return { status: 200, body: visionResponse.content };
  }
});
```
...
```javascript
// Timer trigger (Runs daily at 6am IST) to analyze behavioral logs and generate custom insights
const { app } = require('@azure/functions');
const sql = require('mssql');

app.timer('fitkarma-insights', {
  schedule: '0 30 0 * * *', // Runs at 6am IST
  handler: async (myTimer, context) => {
    const pool = await sql.connect(process.env.SQL_CONNECTION_STRING);
    
    // Ingest all users' rolling metrics to check for behavioral warnings
    const usersResult = await pool.request().query('SELECT localId FROM users WHERE is_active = 1');
    const users = usersResult.recordset;

    for (const user of users) {
      const logsResult = await pool.request()
        .input('userId', sql.VarChar, user.localId)
        .query('SELECT TOP 14 steps, sleep_duration_min, protein_g, date FROM daily_logs WHERE user_id = @userId ORDER BY date DESC');
      ...
      if (stepsDeclined) {
        await pool.request()
          ...
          .query(`
            ...
            VALUES (@userId, 'insight_warning', 'Steps Trend Declining', 'Your steps have dropped consecutively this week. Let\\'s aim for a short 10-minute walk today.', GETUTCDATE())
          `);
      }
      ...
      if (avgSleep < 360) {
        await pool.request()
          ...
          .query(`
            INSERT INTO user_notifications (user_id, type, title, message, created_at)
            VALUES (@userId, 'recovery_alert', 'Sleep Debt Alert', 'Your 14-day average sleep is below 6 hours. Let\\'s push bedtime up by 30 minutes tonight.', GETUTCDATE())
          `);
      }
    }
  }
...
```javascript
// Timer trigger (Runs on the 1st day of every month at midnight UTC) to compile progress reports
const { app } = require('@azure/functions');
const sql = require('mssql');

app.timer('fitkarma-reports', {
  schedule: '0 0 0 1 * *', // 1st of every month
  handler: async (myTimer, context) => {
    const pool = await sql.connect(process.env.SQL_CONNECTION_STRING);
    const usersResult = await pool.request().query('SELECT localId, name, email FROM users WHERE is_active = 1');
    const users = usersResult.recordset;
    ...
    for (const user of users) {
      ...
      const reportData = await pool.request()
        ...
        .query(`
          ...
          FROM daily_logs 
          WHERE user_id = @userId AND date >= DATEADD(day, -30, GETUTCDATE());
        `);
      
      const stats = reportData.recordset[0];
      if (!stats.avgScore) continue;

      // Compile monthly report record
      await pool.request()
        .input('userId', sql.VarChar, user.localId)
        .input('avgSteps', sql.Int, stats.avgSteps || 0)
        .input('avgSleep', sql.Int, stats.avgSleep || 0)
        .input('avgScore', sql.Int, stats.avgScore || 0)
        .query(`
          INSERT INTO monthly_reports (user_id, report_month, avg_steps, avg_sleep_mins, avg_health_score, generated_at)
          VALUES (@userId, MONTH(DATEADD(month, -1, GETUTCDATE())), @avgSteps, @avgSleep, @avgScore, GETUTCDATE());
        `);
    }
  }
});
...
| ADR-003 | **Azure SQL over Cosmos DB** | Azure SQL serverless free tier; SQL schema mirrors Drift local; no per-read billing |
...
| ADR-011 | **LLM via Azure Function** | Keeps Groq key server-side; enables rate limiting, caching, model swapping |
...
- [ ] Azure resource group + Azure SQL (serverless) + Function App created (Central India / Pune)
...
- [ ] `fitkarma-health-os` Azure Timer Function deployed (6am IST)
...
- [ ] `fitkarma-health-os` runs as Durable Functions fan-out, not a sequential per-user loop
...
*FitKarma — Complete Master Documentation*
*Version 1.0 · India's Intelligent Health Operating System*
*Flutter 3.x · Riverpod 2.x · Drift v7 (Local Schema v17) · Azure SQL Serverless · Azure Functions (Durable Functions fan-out) · Entra B2C · RevenueCat · Multi-Model AI (Groq) · Open Food Facts*
*Offline-first · AES-256 encrypted (CSPRNG-keyed) · Privacy-centric · AI-adaptive · Built for India*
*15 development phases · 35+ Azure SQL tables · 8 Azure Functions · 45+ screens · Health OS Brain · Adaptive Metabolism Engine · Longevity Score · 70–95% AI cost reduction*