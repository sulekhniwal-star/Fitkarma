...
void main() {
  ...
  test('AdaptiveMetabolismEngine calculates correct linear regression weight trends', () {
    final now = DateTime.now();
    // Simulate a user losing exactly 0.5 kg per week over 4 weeks
    final weighIns = [
      WeightReading(date: now.subtract(const Duration(days: 21)), weightKg: 80.0),
      WeightReading(date: now.subtract(const Duration(days: 14)), weightKg: 79.5),
      WeightReading(date: now.subtract(const Duration(days: 7)), weightKg: 79.0),
  ...
  test('AdaptiveMetabolismEngine recalibrates TDEE and detects adaptation', () {
    ...
    final weighIns = [
      WeightReading(date: now.subtract(const Duration(days: 21)), weightKg: 80.0),
      WeightReading(date: now.subtract(const Duration(days: 14)), weightKg: 79.9), // Under-performing (losing slowly)
      WeightReading(date: now.subtract(const Duration(days: 7)), weightKg: 79.8),
  ...
  test('AdaptiveMetabolismService updates user dailyCalorieTarget in database', () async {
    final now = DateTime.now();
    final weighIns = [
      WeightReading(date: now.subtract(const Duration(days: 21)), weightKg: 80.0),
      WeightReading(date: now.subtract(const Duration(days: 14)), weightKg: 79.5),
    ...
    final user = await (db.select(db.users)..where((t) => t.id.equals('user_met'))).getSingle();
