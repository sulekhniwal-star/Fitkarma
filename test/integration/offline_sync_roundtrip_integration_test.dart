...
void main() {
  group(
      '§P14-C Integration: Offline -> Online Sync Round-Trip & Conflict Resolution',
      () {
    const resolver = SyncMergeResolver();

    test('LWW Conflict Resolution: Newer local HLC retains local record', () {
      const hlcOlder = HlcTimestamp(
          physicalTimeMillis: 1000, logicalCounter: 0, nodeId: 'nodeA');
      const hlcNewer = HlcTimestamp(
          physicalTimeMillis: 2000, logicalCounter: 0, nodeId: 'nodeB');

      final local =
          TestSyncRecord(id: 'rec_1', content: 'Local Update', hlc: hlcNewer);
      final remote =
          TestSyncRecord(id: 'rec_1', content: 'Remote Update', hlc: hlcOlder);

      final resolution =
          resolver.resolveConflict(localRecord: local, remoteRecord: remote);
      expect(resolution.action, equals(SyncAction.keepLocal));
    });

    test(
        'Cumulative Log CRRA: Merges step deltas from disparate devices deduplicated',
        () {
      const hlc1 = HlcTimestamp(
          physicalTimeMillis: 1000, logicalCounter: 0, nodeId: 'nodeA');
      const hlc2 = HlcTimestamp(
          physicalTimeMillis: 1050, logicalCounter: 0, nodeId: 'nodeB');

      final localSteps = TestStepDeltaLog(
          id: 'steps_today',
          stepCount: 3500,
          syncBatchId: 'batch_local_1',
          hlc: hlc1);
      final remoteSteps = TestStepDeltaLog(
          id: 'steps_today',
          stepCount: 1200,
          syncBatchId: 'batch_remote_2',
          hlc: hlc2);

      final resolution = resolver.resolveConflict(
          localRecord: localSteps, remoteRecord: remoteSteps);
