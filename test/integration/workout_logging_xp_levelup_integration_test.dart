...
void main() {
  group('§P14-C Integration: Workout Logging -> Progressive Overload -> XP & Level Up', () {
    const overloadEngine = ProgressiveOverloadEngine();
    const gamificationEngine = GamificationEngine();

    test('Completed workout sets compute progressive overload targets for next session', () {
    ...
    test('Logging outcome milestone awards Karma XP and triggers Level Up', () {
      ...
      final workoutXp = gamificationEngine.getOutcomeXpReward('workout_completed_full_intensity');
      final programMilestoneXp = gamificationEngine.getOutcomeXpReward('program_week_completed');

      final totalNewXp = initialXp + workoutXp + programMilestoneXp; // 950 + 60 + 150 = 1160 XP
