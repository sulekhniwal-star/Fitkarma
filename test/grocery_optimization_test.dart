...
void main() {
  group('GroceryOptimizationEngine Unit Tests', () {
    ...
    test(
      'getSeedWeeklyIngredients returns baseline Indian high-protein ingredients',
      () {
    ...
    test(
      'optimize keeps raw items when cost is within weekly budget limit',
      () {
    ...
    test(
      'optimize performs protein-per-rupee food swaps when budget is exceeded',
      () {
        final rawItems = GroceryOptimizationEngine.getSeedWeeklyIngredients();
    ...
    test(
      'protein substitute maintains exact protein yield of original item',
      () {
        final rawItems = GroceryOptimizationEngine.getSeedWeeklyIngredients();
        final greekYogurt = rawItems.firstWhere(
          (i) => i.id == 'ing_greek_yogurt',
        );
        ...
        final substitute = result.items.firstWhere(
          (i) => i.isSwappedSubstitute,
        );
  ...
  group('GroceryListScreen & Controller UI Tests', () {
    ...
    testWidgets(
      'renders grocery list screen with budget card and shopping list',
      (tester) async {
        ...
        expect(
          find.byKey(const Key('grocery_monthly_budget_text')),
          findsOneWidget,
        );
    ...
    testWidgets('updating monthly budget re-evaluates optimization state', (
      tester,
    ) async {
      await tester.pumpWidget(_buildApp(container));
      await tester.pump();
    ...
    testWidgets('toggling item purchased checkbox updates item state', (
      tester,
    ) async {
      await tester.pumpWidget(_buildApp(container));
      await tester.pump();

      final firstItemId = container
          .read(groceryProvider)
          .optimizedList!
          .items
          .first
          .id;
      final isPurchasedBefore = container
          .read(groceryProvider)
          .optimizedList!
          .items
          .first
      ...
      final isPurchasedAfter = container
          .read(groceryProvider)
          .optimizedList!
          .items
          .first
    ...
    testWidgets(
      'quick commerce vendor selection updates selected vendor and builds payload (§P16-E)',
      (tester) async {
        await tester.pumpWidget(_buildApp(container));
        await tester.pump();
        ...
        container
            ...
            .setSelectedVendor(VendorPartner.zepto);
        await tester.pump();

        expect(
          container.read(groceryProvider).selectedVendor,
          VendorPartner.zepto,
        );
        ...
        expect(
          find.byKey(const Key('grocery_checkout_payload_box')),
          findsOneWidget,
        );
