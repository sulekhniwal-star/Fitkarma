...
void main() {
  ...
  testWidgets('BpScreen lock screen, unlock via PIN, manual entry, and rising warning check', (tester) async {
    // 1. Pump BpScreen
    ...
    expect(find.text('Sensitive Vitals Locked'), findsOneWidget); // still locked
    ...
    print('DB LOGS IN TEST: ${tempLogs.map((e) => "${e.systolic}/${e.diastolic}").toList()}');
    ...
    expect(find.text('Warning: 3 rising BP readings recorded. Limit caffeine and record again tonight.'), findsOneWidget);
  });
}
