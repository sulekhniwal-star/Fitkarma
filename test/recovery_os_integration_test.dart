...
void main() {
  group('Sleep Intelligence Layer Tests', () {
    test(
      'SleepNeedCalculator incorporates debt, strain, stress, sickness, and caps at 600m',
      () {
    ...
    test(
      'SleepPerformanceScore aggregates duration, efficiency, consistency, and opportunity',
      () {
    ...
    test(
      'BedtimeCoach calculates correct bedtime and generates a personalized nudge',
      () {
        final coach = BedtimeCoach();

        // Sleep need: 500 mins (8h 20m), target wake: 6:30 AM
        final targetWake = DateTime(2026, 7, 18, 6, 30);
        final bedtime = coach.calculateBedtime(
          sleepNeedMins: 500,
  ...
  group('Recovery Capacity & Strain System Tests', () {
    test(
      'DailyStrainCalculator computes exponential strain score from zones and steps',
      () {
    ...
    test(
      'DailyStrainCalculator estimates zones correctly on missing telemetry data',
      () {
        final calculator = DailyStrainCalculator();
    ...
    test(
      'RecoveryDecisionEngine maps readiness score and sleep debt to capacity bounds and advice',
      () {
        final engine = RecoveryDecisionEngine();

        // High capacity scenario
        final decisionHigh = engine.evaluate(
          readinessScore: 90,
          dailyStrain: 5.0,
          sleepDebtHours: 0.0,
        );
        ...
        final decisionLow = engine.evaluate(
          readinessScore: 50,
          dailyStrain: 12.0,
          sleepDebtHours: 2.0,
        );
  ...
  group('Recovery Actionable Behaviors and Sync Tests', () {
    test(
      'RecoveryPrescriptionGenerator yields custom checklist actions based on capacity bounds',
      () {
  ...
  group('Circadian, Environmental, and Drivers Tests', () {
    test(
      'CircadianScoreCalculator tracks midpoint shift penalty and light exposure sync',
      () {
        final calculator = CircadianScoreCalculator();

        // Perfect alignment
        expect(
          calculator.calculateCircadianScore(
            midpointShiftMins: 10,
            morningLightExposure: false,
          ),
          100,
        );
        ...
        expect(
          calculator.calculateCircadianScore(
            midpointShiftMins: 90,
            morningLightExposure: true,
          ),
    ...
    test('IllnessDetector flags sickness deviation trigger', () {
      ...
      expect(
        sickResult.sicknessNudge,
        contains("Warning: Elevated biometric signals"),
      );
    });

    test(
      'RecoveryDriversEngine breaks down contributors and detractors correctly',
      () {
        ...
        final List<dynamic> contributors =
            drivers['contributors'] as List<dynamic>;
        ...
        expect(
          contributors.any((c) => c['driver'] == 'Hydration Target'),
          true,
        );
  ...
  group('Recovery OS Integration Pipeline Test', () {
    test(
      'Simulated user pipeline workflow calculates complete recovery profile',
      () {
        ...
        final bedtime = coach.calculateBedtime(
          sleepNeedMins: sleepNeedMins,
          targetWakeTime: wakeTime,
        );
        final nudge = coach.generateNudge(
          sleepNeedMins: sleepNeedMins,
          bedtime: bedtime,
        );
        ...
        final prescription = prescriptionGen.generate(
          capacityScore: decision.capacityScore,
        );
        expect(
          prescription.any((p) => p.contains("mobility")),
          true,
        ); // standard capacity checklist
