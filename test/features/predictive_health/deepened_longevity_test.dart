...
void main() {
  group('DeepenedLongevityEngine Tests', () {
    ...
    test('Synthesizes optimal cellular resilience & all 7 hallmarks of aging',
      ...
      expect(
          report.compositeCellularResilienceScore, greaterThanOrEqualTo(85.0));
      expect(report.hallmarkEvaluations.length, equals(7));

      // Check all 7 hallmarks
      final mito = report.hallmarkEvaluations
          .firstWhere((h) => h.hallmark == HallmarkOfAging.mitochondrialHealth);
      expect(mito.score, greaterThanOrEqualTo(90.0));
      expect(mito.isProtective, isTrue);

      final proto = report.hallmarkEvaluations
          .firstWhere((h) => h.hallmark == HallmarkOfAging.proteostasisAndAges);
      expect(proto.score, greaterThanOrEqualTo(85.0));
      expect(proto.isProtective, isTrue);

      final stem = report.hallmarkEvaluations.firstWhere(
          (h) => h.hallmark == HallmarkOfAging.stemCellRegeneration);
      ...
      expect(
          report.cellularRoadmap[1].ayurvedicRasayana, contains('Ashwagandha'));
      expect(report.cellularRoadmap[2].ayurvedicRasayana,
          contains('Chyawanprash'));
    });

    test(
        'Calibrates Thin-Fat phenotype risk penalties for high visceral fat and elevated Lp(a)',
        () {
      final report = engine.synthesizeDeepenedLongevity(
      ...
      expect(report.southAsianRisk.clinicalInterpretation,
          contains('Thin-Fat phenotype profile detected'));
      expect(report.southAsianRisk.sarcopenicRiskIndex, greaterThan(50.0));

      final infl = report.hallmarkEvaluations
          .firstWhere((h) => h.hallmark == HallmarkOfAging.inflammaging);
  ...
  group('DeepenedLongevityNotifier Tests', () {
    test('Initializes with baseline report and recalculates dynamically', () {
      ...
      expect(
          notifier.state.compositeCellularResilienceScore, greaterThan(90.0));
    });
  });
}
