...
void main() {
  group('§P10-I Medication Tracker & Interaction Warning Engine Tests', () {
    ...
    test('checkSchedule triggers warning for Metformin when meal carbs > 80g', () {
      ...
      const meal = MealSnapshot(mealName: 'High Carb Meal', carbsGrams: 95.0, foodItems: ['Rice']);

      final warnings = engine.checkSchedule(med, meal, WorkoutIntensityLevel.moderate);

      expect(warnings.length, equals(1));
      expect(warnings.first.severity, equals(InteractionSeverity.moderate));
      expect(warnings.first.message, contains('Metformin combined with high simple carbs'));
    });

    test('checkSchedule triggers high severity warning for Statins with Grapefruit', () {
      final med = MedicationSchedule(
        localId: 'm2',
      ...
      const meal = MealSnapshot(mealName: 'Citrus Meal', carbsGrams: 30.0, foodItems: ['Grapefruit Juice']);

      final warnings = engine.checkSchedule(med, meal, WorkoutIntensityLevel.low);

      expect(warnings.length, equals(1));
      expect(warnings.first.severity, equals(InteractionSeverity.high));
      expect(warnings.first.message, contains('Grapefruit compounds inhibit metabolic enzymes'));
    });

    test('resolveOfflineRxcui maps Indian brand names to RxNorm Concept IDs (RxCUIs)', () {
    ...
    testWidgets('MedicationTrackerScreen renders scheduled medications and active interaction warnings', (tester) async {
      ...
      expect(find.textContaining('Metformin combined with high simple carbs'), findsOneWidget);
