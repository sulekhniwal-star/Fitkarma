...
void main() {
  group('§P10-L Retrospective Glycemic Processing Pipeline Tests', () {
    ...
    test(
        'processMealWindow returns complete result when baseline and post-meal readings satisfy density requirements',
        () {
      ...
      final readings = [
        ...
        CgmReadingData(
            id: 'r1',
            timestamp: mealTime.subtract(const Duration(minutes: 25)),
            glucoseMgDl: 90.0),
        CgmReadingData(
            id: 'r2',
            timestamp: mealTime.subtract(const Duration(minutes: 10)),
            glucoseMgDl: 94.0),
        // Post-meal window (0m to +120m)
        CgmReadingData(
            id: 'r3',
            timestamp: mealTime.add(const Duration(minutes: 20)),
            glucoseMgDl: 120.0),
        CgmReadingData(
            id: 'r4',
            timestamp: mealTime.add(const Duration(minutes: 45)),
            glucoseMgDl: 142.0),
        CgmReadingData(
            id: 'r5',
            timestamp: mealTime.add(const Duration(minutes: 70)),
            glucoseMgDl: 130.0),
        CgmReadingData(
            id: 'r6',
            timestamp: mealTime.add(const Duration(minutes: 95)),
            glucoseMgDl: 105.0),
      ];

      final result = matcher.processMealWindow(
          mealConsumeTime: mealTime, syncedReadings: readings);
    ...
    test(
        'processMealWindow returns incomplete result when reading density is insufficient',
        () {
      final now = DateTime.now();
      final mealTime = now.subtract(const Duration(hours: 2));
    ...
    testWidgets(
        'RetrospectiveGlycemicPipelineScreen renders pipeline overview and retrospective insight cards',
        (tester) async {
      ...
      expect(
          find.textContaining('Lunch (Dal Makhani + 2 Rotis)'), findsOneWidget);
