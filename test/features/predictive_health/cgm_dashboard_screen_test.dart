...
void main() {
  group('§P10-H Continuous Biomarker Tracking (CGM Sync) Tests', () {
    ...
    test('detectSpikes detects spike >40 mg/dL within 90 mins and correlates pre-spike foods', () {
      final now = DateTime.now();
      final readings = [
        GlucoseReading(readingId: 'g1', timestamp: now.subtract(const Duration(hours: 2)), glucoseValueMgDl: 120.0, trendDirection: GlucoseTrend.rising),
        GlucoseReading(readingId: 'g2', timestamp: now.subtract(const Duration(hours: 1, minutes: 45)), glucoseValueMgDl: 172.0, trendDirection: GlucoseTrend.rapidlyRising),
      ];
      ...
      expect(spikes.first.correlatedFoods.first.foodName, equals('White Rice (Thali)'));
    });

    // ── Widget Tests ────────────────────────────────────────────────────────

    testWidgets('CgmDashboardScreen renders Live Glucose Stream card and detected spikes breakdown', (tester) async {
      ...
      expect(find.textContaining('White Rice (Thali)'), findsAtLeastNWidgets(1));
    });
  });
}
