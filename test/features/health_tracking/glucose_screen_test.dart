...
void main() {
  group('§P4-E Glucose Screen & HbA1c Estimation Tests', () {
    ...
    test('categorizeGlucose: Fasting category thresholds', () {
      expect(GlucoseEngine.categorizeGlucose(95, GlucoseContextTag.fasting), equals(GlucoseCategory.normal));
      expect(GlucoseEngine.categorizeGlucose(105, GlucoseContextTag.fasting), equals(GlucoseCategory.elevated));
      expect(GlucoseEngine.categorizeGlucose(130, GlucoseContextTag.fasting), equals(GlucoseCategory.high));
    });

    test('categorizeGlucose: Post-meal category thresholds', () {
      expect(GlucoseEngine.categorizeGlucose(120, GlucoseContextTag.postMeal1h), equals(GlucoseCategory.normal));
      expect(GlucoseEngine.categorizeGlucose(155, GlucoseContextTag.postMeal1h), equals(GlucoseCategory.elevated));
      expect(GlucoseEngine.categorizeGlucose(210, GlucoseContextTag.postMeal2h), equals(GlucoseCategory.critical));
    });

    test('calculateEstimatedHba1c: mathematical formula calculation ((Avg + 46.7) / 28.7)', () {
      final records = [
        GlucoseRecord(mgDl: 100, tag: GlucoseContextTag.fasting, measuredAt: DateTime.now()),
        GlucoseRecord(mgDl: 140, tag: GlucoseContextTag.postMeal1h, measuredAt: DateTime.now()),
      ];
      // Avg = 120 mg/dL → HbA1c = (120 + 46.7)/28.7 = 5.808 -> 5.8%
      final result = engine.calculateEstimatedHba1c(records: records, totalLoggedDays: 95);
    ...
    test('calculateEstimatedHba1c: requires >= 90 days of logs per spec', () {
      final records = [
        GlucoseRecord(mgDl: 95, tag: GlucoseContextTag.fasting, measuredAt: DateTime.now()),
      ];
      final result = engine.calculateEstimatedHba1c(records: records, totalLoggedDays: 45);
    ...
    test('GlucoseNotifier initializes with sample records and unlocked state after microtask', () {
    ...
    test('GlucoseNotifier.logGlucoseReading updates records and recalculates HbA1c', () {
      final notifier = GlucoseNotifier(const GlucoseEngine());
      ...
      expect(notifier.state.latestPostMeal?.correlatedMealName, equals('Biryani'));
      expect(notifier.state.spikeNudge, contains('Biryani'));
    });

    // ── Widget Tests ────────────────────────────────────────────────────────

    testWidgets('GlucoseScreen renders Blood Glucose title and main widgets', (tester) async {
