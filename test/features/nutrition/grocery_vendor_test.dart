...
void main() {
  group('GroceryVendorEngine Unit Tests', () {
    test('generateDefaultCart generates vegetarian cart with ayurvedic essentials', () {
    ...
    test('generateDefaultCart includes non-veg protein sources when isVegetarian is false', () {
    ...
    test('compileCheckoutPayload compiles quotes for all 6 vendors', () {
      ...
      final blinkitQuote = payload.vendorQuotes.firstWhere((q) => q.vendor == GroceryVendorType.blinkit);
      final zeptoQuote = payload.vendorQuotes.firstWhere((q) => q.vendor == GroceryVendorType.zepto);
      final bbQuote = payload.vendorQuotes.firstWhere((q) => q.vendor == GroceryVendorType.bigBasket);

      expect(blinkitQuote.eta, equals('10-12 mins'));
      expect(blinkitQuote.deepLinkUrl.startsWith('blinkit://search?q='), isTrue);
      expect(zeptoQuote.deepLinkUrl.startsWith('zepto://search?query='), isTrue);
    ...
    test('compileCheckoutPayload correctly marks cheapest and fastest options', () {
      ...
      final cheapestCount = payload.vendorQuotes.where((q) => q.isCheapest).length;
      expect(cheapestCount, greaterThanOrEqualTo(1));

      final fastestQuotes = payload.vendorQuotes.where((q) => q.isFastest).toList();
      expect(fastestQuotes.isNotEmpty, isTrue);
      expect(fastestQuotes.any((q) => q.vendor == GroceryVendorType.blinkit), isTrue);
    });

    test('compileCheckoutPayload generates bilingual WhatsApp Kirana share text', () {
      final items = GroceryVendorEngine.generateDefaultCart();
      final payload = GroceryVendorEngine.compileCheckoutPayload(items: items, pincode: '110001');

      expect(payload.whatsappShareText, contains('FitKarma Smart Grocery List'));
    ...
    test('Toggle items updates activeItemCount and protein totals correctly', () {
      final items = GroceryVendorEngine.generateDefaultCart();
      final originalPayload = GroceryVendorEngine.compileCheckoutPayload(items: items);

      final toggledItems = items.map((i) => i.id == 'soya_chunks' ? i.copyWith(isChecked: false) : i).toList();
      final newPayload = GroceryVendorEngine.compileCheckoutPayload(items: toggledItems);

      expect(newPayload.activeItemCount, equals(originalPayload.activeItemCount - 1));
      expect(newPayload.totalProteinYieldGrams, lessThan(originalPayload.totalProteinYieldGrams));
    });
  });
}
