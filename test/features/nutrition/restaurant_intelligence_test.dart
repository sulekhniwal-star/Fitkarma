...
void main() {
  group('§P5-E Indian Restaurant Intelligence 2.0 Tests', () {
    ...
    test('MajorChainPresetsDatabase contains seeded presets for top Indian chains', () {
      expect(MajorChainPresetsDatabase.presets.length, equals(5));

      final haldiram = MajorChainPresetsDatabase.presets.firstWhere((p) => p.chainName == "Haldiram's");
    ...
    test('parseMenuText matches OCR lines and assigns goal overlay colors', () {
      ...
      expect(results[0].colorOverlay, equals(MenuGoalOverlayCategory.greenHighProtein)); // Paneer Tikka > 20g Pro
      expect(results[1].colorOverlay, equals(MenuGoalOverlayCategory.redAvoidAlert));     // Chole Bhature deep fried
      expect(results[2].colorOverlay, equals(MenuGoalOverlayCategory.blueLowCalorie));   // Sprouted Moong < 300 kcal
      expect(results[3].colorOverlay, equals(MenuGoalOverlayCategory.neutral));          // Unmatched -> neutral
    });

    test('matchDishInDatabase fuzzy matches OCR lines using token containment and Levenshtein distance', () {
    ...
    testWidgets('RestaurantMenuScanScreen renders OCR Scanner, Legend, and Chain Presets', (tester) async {
      ...
      expect(find.text('Major Indian Chain Optimization Presets:'), findsOneWidget);
