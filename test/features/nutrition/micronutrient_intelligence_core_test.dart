...
void main() {
  group('§P5-I Micronutrient Intelligence Core Tests', () {
    ...
    test('UserMicroTargets derives RDA, 1.8x Non-Heme Veg Iron, and Female/PCOS target adjustments', () {
      final maleOmnivore = UserMicroTargets.derive(isFemale: false, isVegetarian: false, hasPcosOrFertilityGoal: false);
      expect(maleOmnivore.targetIronMg, equals(8.0));
      expect(maleOmnivore.targetB12Mcg, equals(2.4));

      final vegMale = UserMicroTargets.derive(isFemale: false, isVegetarian: true, hasPcosOrFertilityGoal: false);
      ...
      final femalePcos = UserMicroTargets.derive(isFemale: true, isVegetarian: false, hasPcosOrFertilityGoal: true);
    ...
    test('evaluateLogs triggers B12 depletion risk alert for vegetarian with <50% intake', () {
      final targets = UserMicroTargets.derive(isFemale: false, isVegetarian: true, hasPcosOrFertilityGoal: false);
      final logs = [
      ...
      expect(alerts.firstWhere((a) => a.title.contains('B12 Depletion')).severity, equals(MicronutrientAlertSeverity.high));
    });

    test('evaluateLogs triggers Iron Deficit Warning for female with <60% intake', () {
      final targets = UserMicroTargets.derive(isFemale: true, isVegetarian: false, hasPcosOrFertilityGoal: false);
      final logs = [
      ...
      expect(alerts.firstWhere((a) => a.title.contains('Iron Deficit')).severity, equals(MicronutrientAlertSeverity.medium));
    });

    // ── Widget Tests ────────────────────────────────────────────────────────

    testWidgets('MicronutrientIntelligenceScreen renders controls, auto-alerts, and target table', (tester) async {
