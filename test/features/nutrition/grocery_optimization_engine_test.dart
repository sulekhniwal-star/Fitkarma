...
void main() {
  group('§P5-F Grocery Optimization Engine 2.0 Tests', () {
    ...
    test(
        'proteinCostIndex maps soya_chunks, eggs, curd, paneer INR per gram of protein correctly',
        () {
      expect(GroceryOptimizationEngine.proteinCostIndex['soya_chunks'],
          equals(0.15));
      expect(GroceryOptimizationEngine.proteinCostIndex['eggs'], equals(0.38));
      expect(GroceryOptimizationEngine.proteinCostIndex['double_toned_curd'],
          equals(0.45));
      expect(
          GroceryOptimizationEngine.proteinCostIndex['paneer'], equals(0.75));
    });

    test(
        'optimize performs protein-per-rupee swaps when raw cost exceeds monthly budget',
        () {
    ...
    test(
        'optimize edge-case: generates clear warning when budget is too low even after swaps',
        () {
      final result = engine.optimize(
        weekPlan: expensivePlan,
    ...
    testWidgets(
        'GroceryOptimizationScreen renders budget sliders, cost summaries, and swap list',
        (tester) async {
