...
void main() {
  group('§P5-P Satiety Prediction Engine Tests', () {
    ...
    test('computeForSeededItem validates Paneer Bhurji (90/100) & Rajma Chawal (85/100) per §P5-P spec', () {
      final paneerBhurji = SeededIndianSatietyTable.items.firstWhere((i) => i.id == 'sat_1');
      final paneerScore = engine.computeForSeededItem(paneerBhurji);
      expect(paneerScore.satietyScore, greaterThanOrEqualTo(85.0));

      final rajmaChawal = SeededIndianSatietyTable.items.firstWhere((i) => i.id == 'sat_2');
      final rajmaScore = engine.computeForSeededItem(rajmaChawal);
      expect(rajmaScore.satietyScore, greaterThanOrEqualTo(85.0));
    });

    test('computeForSeededItem validates Deep-Fried Samosa (30/100) vs Air-Fried Samosa (60/100)', () {
      final airFried = SeededIndianSatietyTable.items.firstWhere((i) => i.id == 'sat_3');
      final airScore = engine.computeForSeededItem(airFried);
      expect(airScore.satietyScore, greaterThanOrEqualTo(50.0));

      final deepFried = SeededIndianSatietyTable.items.firstWhere((i) => i.id == 'sat_4');
    ...
    testWidgets('SatietyPredictionScreen renders dropdown selector, satiety score card, and reference list', (tester) async {
