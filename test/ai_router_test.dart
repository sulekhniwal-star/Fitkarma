...
void main() {
  ...
  test(
    'AICache handles composite key mapping, expiration, and user purging',
    () async {
      ...
      await db
          .update(db.aICacheEntries)
          .write(AICacheEntriesCompanion(expiresAt: Value(yesterday)));
  ...
  test(
    'RuleEngine triggers hydration warning when logs are low past 3PM',
    () async {
  ...
  test(
    'AIRouter selects model tier, retries flaky networks, and triggers fallbacks',
    () async {
      final request = AIRequest(
        userId: 'user_001',
      ...
      expect(
        response,
        contains('llama3_8b'),
      ); // Tiny model classification chosen

      // Test retry policy on flaky Groq endpoint
      router.simulateFlakyGroq =
          true; // Will fail 2 times, succeed on 3rd attempt
      ...
      expect(
        response2,
        contains('llama3_70b_full'),
      ); // Large model chosen, retry succeeds
      ...
      expect(
        offlineResponseHigh,
        contains('[Local Gemma-2B Offline Response]'),
      );
