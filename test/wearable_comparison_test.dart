...
void main() {
  group('DeviceReliabilityEngine Unit Tests', () {
    ...
    test('WHOOP 4.0 has 0.95 HRV confidence and 1.0 weight', () {
      final res = engine.applyConfidence(source: WearableSource.whoop, rawHRV: 60.0, rawHR: 70.0);
    ...
    test('Garmin has 0.88 HRV confidence and 1.0 weight', () {
      final res = engine.applyConfidence(source: WearableSource.garmin, rawHRV: 60.0, rawHR: 70.0);
      expect(res.hrvConfidence, 0.88);
      expect(res.readinessWeight, 1.0);
    });

    test('Samsung Watch 6 has 0.70 HRV confidence and 0.70 weight', () {
      final res = engine.applyConfidence(source: WearableSource.samsungWatch, rawHRV: 60.0, rawHR: 70.0);
      expect(res.hrvConfidence, 0.70);
      expect(res.readinessWeight, 0.70);
    });

    test('Mi Band / Noise has 0.40 HRV confidence and 0.40 weight', () {
      final res = engine.applyConfidence(source: WearableSource.miBand, rawHRV: 60.0, rawHR: 70.0);
  ...
  group('WearableSyncMerger Unit Tests', () {
    ...
    test('Cardiac Data merges and resolves overlapping minute buckets using higher confidence', () {
      ...
      final incoming = [
        WearableDataPoint(
          timestamp: timestamp.add(const Duration(seconds: 15)), // Same minute bucket
    ...
    test('Cumulative Steps overrides lower-confidence phone counts for same hour bucket', () {
      final t1 = DateTime(2026, 7, 19, 8, 30);
      
      final local = [
        WearableDataPoint(
          timestamp: t1,
          source: WearableSource.manual, // Phone step estimation (low confidence)
  ...
  group('WearableComparisonScreen Widget Tests', () {
    Widget buildSubject() {
      return const MaterialApp(
        home: WearableComparisonScreen(),
      );
    }

    testWidgets('Renders WearableComparisonScreen and updates selected source details', (tester) async {
      await tester.pumpWidget(buildSubject());
      await tester.pumpAndSettle();

      // Verify title and dropdown exists
      expect(find.text('Wearable Comparison'), findsOneWidget);
      expect(find.byKey(const Key('wearable_source_dropdown')), findsOneWidget);

      // Verify default primary device display card (WHOOP 4.0)
      expect(find.byKey(const Key('wearable_primary_source_card')), findsOneWidget);
      expect(find.textContaining('HRV Source: WHOOP 4.0'), findsOneWidget);
      expect(find.textContaining('weighted at 100% confidence'), findsOneWidget);
      ...
      expect(find.textContaining('HRV Source: Samsung Galaxy Watch 6'), findsOneWidget);
      expect(find.textContaining('weighted at 70% confidence'), findsOneWidget);
    });
  });
}
