...
void main() {
  group('§P6-C ProgressiveOverloadEngine & PoseEstimationEngine Tests', () {
    ...
    test('suggestMultiSessionProgression suggests +2.5kg jump after 3 consecutive comfortable sessions (RPE <= 7)', () {
      const exercise = ExerciseTarget(name: 'Bench Press', currentWeight: 80.0, nextWeightStep: 2.5);
      final sessions = [
        WorkoutSessionRecord(date: DateTime.now().subtract(const Duration(days: 7)), repsTarget: 8, repsCompleted: 8, rpe: 6.5, weightKg: 80.0),
        WorkoutSessionRecord(date: DateTime.now().subtract(const Duration(days: 4)), repsTarget: 8, repsCompleted: 8, rpe: 7.0, weightKg: 80.0),
        WorkoutSessionRecord(date: DateTime.now().subtract(const Duration(days: 1)), repsTarget: 8, repsCompleted: 8, rpe: 6.8, weightKg: 80.0),
      ];
      ...
      expect(suggestion.progressionType, equals(ProgressionType.increaseWeight));
      expect(suggestion.recommendationReason, contains('Increase to 82.5kg'));
    });

    test('suggestMultiSessionProgression triggers deload week at 60% intensity upon 4-week plateau', () {
      const exercise = ExerciseTarget(name: 'Squat', currentWeight: 100.0, nextWeightStep: 2.5);
      ...
      expect(suggestion.recommendationReason, contains('deload week at 60% intensity'));
    });
