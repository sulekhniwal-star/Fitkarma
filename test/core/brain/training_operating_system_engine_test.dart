...
void main() {
  group('§P6-E Training Operating System Tests', () {
    ...
    test(
        'diagnoseSquatPattern identifies dorsiflexion and valgus faults and prescribes correctives per §P6-E spec',
        () {
      ...
      expect(report.prescribedDrills.any((d) => d.contains('ankle rocker')),
          isTrue);
      expect(report.prescribedDrills.any((d) => d.contains('resistance band')),
          isTrue);
      expect(report.mobilityIndex, lessThan(100));
    });

    test(
        'calculateExerciseConfidenceScore (ECS) computes confidence index strictly clamped to 0-100',
        () {
    ...
    test(
        'calculateMovementAge calculates younger Movement Age for high MHS score',
        () {
      final profile =
          engine.calculateMovementAge(actualAge: 30, mhsScore: 88.0);
    ...
    test(
        'selectAlternativeExercise performs smart swaps for joint mobility limitations',
        () {
    ...
    test(
        'calculateLocalSegmentReadiness flags leg recovery needed when lower readiness is low',
        () {
    ...
    test(
        'analyzeUnilateralRep detects asymmetry imbalance when delta exceeds 10%',
        () {
    ...
    test(
        'forecastStrength projects 8-week and 12-week load progression based on training reliability',
        () {
      ...
      expect(
          forecast.forecastSummary, contains('progress your load to 82.0 kg'));
    });

    // ── Widget Tests ────────────────────────────────────────────────────────

    testWidgets(
        'TrainingOsScreen renders MHS score, Local Readiness card, Mobility card, and Forecasting card',
        (tester) async {
      ...
      expect(
          find.text('Biomechanical Asymmetry & Forecasting'), findsOneWidget);
    });
  });
}
