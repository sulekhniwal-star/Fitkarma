...
void main() {
  group('§P12-F Calendar Intelligence Engine Tests (Pure Dart)', () {
    ...
    test(
        'Heavy meeting schedule (>6h / >360m) adapts workout to 20-min HIIT meeting-day protocol',
        () async {
      final heavySchedule = [
        CalendarEvent(
            id: '1',
      ...
      expect(insight.workoutRecommendation.type,
          equals('20-min HIIT (meeting-day protocol)'));
      expect(
          insight.workoutRecommendation.rationale,
          contains(
              'Heavy meeting day — shortened workout better than skipping entirely.'));
      expect(insight.nutritionNote,
          contains('Heavy cognitive load day → craving more carbs is normal.'));
      expect(insight.nutritionNote,
          contains('Keep a healthy snack nearby to avoid vending machine.'));
    });

    test('Detects wedding event and triggers 20-min light morning workout',
        () async {
      final weddingSchedule = [
        CalendarEvent(
            id: '1',
            title: 'Priya & Rohan Wedding Reception',
            startHour: 17,
            durationMinutes: 240,
            date: now),
      ];
      ...
      expect(insight.workoutRecommendation.isAdapted, isTrue);
      expect(insight.workoutRecommendation.type,
          equals('Light 20-min morning workout'));
      expect(
          insight.workoutRecommendation.rationale,
          contains(
              'Wedding day — brief workout keeps energy high without fatigue for the celebration.'));
      expect(insight.nutritionNote,
          contains('Wedding celebration: Prioritize hydration'));
    });

    test('Detects travel event and sets travel nutrition hydration note',
        () async {
      final travelSchedule = [
        CalendarEvent(
            id: '1',
            title: 'Flight to Bengaluru Airport',
            startHour: 7,
            durationMinutes: 180,
            date: now),
      ];
    ...
    test('Standard schedule (<300m) maintains standard 45-min workout',
        () async {
      final lightSchedule = [
        CalendarEvent(
            id: '1',
            title: 'Quick Daily Sync',
            startHour: 10,
            durationMinutes: 30,
            date: now),
        CalendarEvent(
      ...
      expect(insight.workoutRecommendation.type,
          equals('45-min strength session'));
      expect(insight.nutritionNote,
          contains('Standard schedule: Maintain your regular meal timings'));
    });

    test('Detects morning commitments (<9am) and evening commitments (>=6pm)',
        () async {
      final schedule = [
        CalendarEvent(
            id: '1',
            title: 'Early Global Call',
            startHour: 8,
            durationMinutes: 45,
            date: now),
        CalendarEvent(
