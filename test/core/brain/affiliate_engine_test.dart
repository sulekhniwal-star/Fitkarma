...
void main() {
  group('§P13-C Creator Affiliate Engine Tests (Pure Dart)', () {
    ...
    test('Calculates 15% recurring affiliate commission accurately', () {
      ...
      final annualCommission = engine.calculateCommission(saleAmountInr: 1999.0);
      expect(annualCommission, equals(299.85));

      // Monthly plan: ₹299 -> 15% = ₹44.85
      final monthlyCommission = engine.calculateCommission(saleAmountInr: 299.0);
      expect(monthlyCommission, equals(44.85));
    });

    test('Calculates 10% client referral discount accurately', () {
      final annualDiscount = engine.calculateDiscount(originalAmountInr: 1999.0);
    ...
    test('processInstantPayout generates a new paid entry and zeroes available balance', () {
    ...
    test('AffiliateNotifier updates referral promo code and processes bank transfers', () async {
      final notifier = AffiliateNotifier(engine);

      expect(notifier.state.referralLink.referralCode, equals('SHARMA10'));
      expect(notifier.state.referralLink.fullUrl, equals('fitkarma.com/ref/sharma10'));
      ...
      expect(notifier.state.referralLink.fullUrl, equals('fitkarma.com/ref/fitdelhi'));
