...
void main() {
  group('§P13-B Double-Entry Ledger & Compliance Engine Tests (Pure Dart)', () {
    test(
        'Calculates exact 80/20 split, 18% GST on platform fee, 1% TCS, and 1% TDS',
        () {
      ...
      expect(ledger.sumAccountBalance(LedgerAccountType.cashAsset),
          equals(1000.0));

      // Verify platform 20% commission fee = ₹200
      // Platform Revenue credited ₹200 (fee) + ₹36 (GST offset) = ₹236
      expect(ledger.sumAccountBalance(LedgerAccountType.platformRevenue),
          equals(236.0));

      // Verify 18% GST on platform commission = ₹36 (18% of ₹200)
      expect(
          ledger.sumAccountBalance(LedgerAccountType.gstExpense), equals(36.0));

      // Verify 1% TCS liability = ₹10
      expect(ledger.sumAccountBalance(LedgerAccountType.tcsLiability),
          equals(10.0));

      // Verify 1% TDS liability = ₹10
      expect(ledger.sumAccountBalance(LedgerAccountType.tdsLiability),
          equals(10.0));

      // Verify creator net escrow = 80% (₹800) - 1% TCS (₹10) - 1% TDS (₹10) = ₹780
      expect(ledger.sumAccountBalance(LedgerAccountType.escrowLiability),
          equals(780.0));
    });
    ...
    test('Releases escrow to creator payable after 7-day dispute window clears',
      ...
      expect(ledger.sumAccountBalance(LedgerAccountType.escrowLiability),
          equals(netEscrow));
      expect(ledger.sumAccountBalance(LedgerAccountType.creatorPayable),
          equals(0.0));
    ...
    test('Processes dispute refund in escrow window correctly', () {
      ...
      expect(
          ledger.sumAccountBalance(LedgerAccountType.cashAsset), equals(0.0));
      expect(ledger.sumAccountBalance(LedgerAccountType.escrowLiability),
          equals(0.0));
      expect(ledger.sumAccountBalance(LedgerAccountType.tcsLiability),
          equals(0.0));
      expect(ledger.sumAccountBalance(LedgerAccountType.tdsLiability),
          equals(0.0));
      expect(
          ledger.sumAccountBalance(LedgerAccountType.gstExpense), equals(0.0));
    });

    test(
        'WebhookSignatureVerifier verifies genuine HMAC-SHA256 signature and rejects spoofed headers',
        () {
      ...
      expect(
        verifier.verifyRazorpaySignature(
            payload: payload, signatureHeader: '', secret: secret),
        isFalse,
      );
    });

    test(
        'CoachMatchingEngine ranks coaches correctly matching user goals and verified credentials',
        () {
