...
void main() {
  group('§P7-A Karma System Design & Outcome XP Tests', () {
    ...
    test(
        'getOutcomeXpReward awards XP ONLY for health outcomes and milestones per §P7-A spec',
        () {
      expect(engine.getOutcomeXpReward('protein_target_met'), equals(50));
      expect(engine.getOutcomeXpReward('sleep_streak_7d'), equals(80));
      expect(
          engine.getOutcomeXpReward('readiness_improved_weekly'), equals(100));
      expect(engine.getOutcomeXpReward('workout_completed_full_intensity'),
          equals(60));
    ...
    test(
        'getOutcomeXpReward strictly returns ZERO XP for logging inputs (anti-perverse incentive rule)',
        () {
    ...
    test(
        'calculateLevel correctly maps Karma Levels and titles per §P7-A spec table',
        () {
