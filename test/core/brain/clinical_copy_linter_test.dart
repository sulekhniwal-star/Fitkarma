...
void main() {
  group('§P10-M Clinical Copy Directive-Language Linter Tests', () {
    ...
    test(
        'lint catches banned directive phrases (stop taking, do not take, switch to, reduce your dose)',
        () {
      expect(linter.lint('Please stop taking your statin immediately.'),
          hasLength(1));
      expect(
          linter.lint('Do not take metformin with high carbs.'), hasLength(1));
    ...
    test('lint allows "avoid" when "consult" IS present nearby', () {
      expect(
          linter.lint('Consult your doctor to avoid grapefruit interaction.'),
          isEmpty);
    });

    test(
        'lint verifies all active interaction warning strings in Medication Tracker Engine',
        () {
      ...
      final meal = MealSnapshot(
          mealName: 'Lunch', carbsGrams: 90.0, foodItems: ['Grapefruit']);

      final warnings =
          engine.checkSchedule(med, meal, WorkoutIntensityLevel.high);
      ...
      for (final warn in warnings) {
        ...
        expect(violations, isEmpty,
            reason: 'Violation found in warning: ${warn.message}');
      }
    });
  });
}
