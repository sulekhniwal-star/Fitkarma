...
void main() {
  group('§P8-A Transformation Journey Engine Tests', () {
    ...
    test('TransformationMemory initializes longitudinal memory fields correctly', () {
    ...
    test('ConsistencyTracker returns moderate when riskScore is 2 or 3', () {
      ...
      expect(engine.analyzeConsistency(data), equals(ConsistencyStatus.moderate));
    });
    ...
    test('ConsistencyTracker returns highRelapse when riskScore >= 4', () {
      ...
      expect(engine.analyzeConsistency(data), equals(ConsistencyStatus.highRelapse));
    });

    test('RelapseIntervention returns Day 1 gentle nudge for 1 missed day', () {
      final intervention = engine.getRelapseIntervention(consecutiveMissedDays: 1);
      expect(intervention.tier, equals(RelapseTier.day1GentleNudge));
      expect(intervention.message, contains('10-minute walk today counts as a win'));
    });

    test('RelapseIntervention returns Day 2 Lite plan switch for 2 missed days', () {
      final intervention = engine.getRelapseIntervention(consecutiveMissedDays: 2);
      expect(intervention.tier, equals(RelapseTier.day2PlanAdjustment));
      expect(intervention.message, contains('switched you to the Lite Plan for 3 days'));
    });

    test('RelapseIntervention returns Day 3 emergency reframe for 3 missed days', () {
      final intervention = engine.getRelapseIntervention(consecutiveMissedDays: 3, previousStreakDays: 14);
      expect(intervention.tier, equals(RelapseTier.day3EmergencyReframe));
      expect(intervention.message, contains('doesn\'t erase your 14-day streak'));
    });

    test('RelapseIntervention returns Day 5 squad connection for 5 missed days', () {
      final intervention = engine.getRelapseIntervention(consecutiveMissedDays: 5, squadMemberName: 'Ananya');
