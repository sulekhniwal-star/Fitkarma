...
void main() {
  group('§P7-D Adherence Score Calculator & Widget Tests', () {
    ...
    final workoutLogs = List.generate(
      3,
      (i) => WorkoutLogItem(
          completionPercent: 90.0, date: now.subtract(Duration(days: i))),
    );
    ...
    test(
        'calculate computes 40% Nutrition + 40% Training + 20% Recovery adherence score per §P7-D formula',
        () {
    ...
    test('Adherence >= 90% unlocks +50% XP multiplier (1.5x)', () {
      ...
      final perfectWorkout = List.generate(
        4,
        (i) => WorkoutLogItem(
            completionPercent: 100.0, date: now.subtract(Duration(days: i))),
      );
    ...
    testWidgets(
        'AdherenceScoreWidget renders breakdown bars, weakest area, and boost badges',
        (tester) async {
      final res = calculator.calculate(
        foodLogs: foodLogs,
        workoutLogs: workoutLogs,
