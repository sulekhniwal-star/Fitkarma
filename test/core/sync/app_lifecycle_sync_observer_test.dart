...
void main() {
  group('§P14-B App Lifecycle Sync Observer Tests (Pure Dart)', () {
    test('Executes 3-step synchronization sequence in exact order upon resume after cooldown', () async {
      ...
      expect(mockCoordinator.callOrder, equals([
    ...
    test('Suppresses sync if app was resumed within the 5-minute cooldown period', () async {
      final mockCoordinator = MockSyncCoordinator();
    ...
    test('Handles silent push notification with content-available payload in background', () async {
      final mockCoordinator = MockSyncCoordinator();
      final observer = AppLifecycleSyncObserver(mockCoordinator);

      // Non-content-available push -> ignored
      final ignored = await observer.handleSilentPushNotification({'title': 'Regular Alert'});
      expect(ignored, isFalse);
      expect(observer.syncExecutionCount, equals(0));

      // Silent push wake-up
      final handled = await observer.handleSilentPushNotification({'content-available': 1});
