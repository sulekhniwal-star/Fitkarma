...
void main() {
  group('SmartFestivalNutritionEngine Unit Tests', () {
    ...
    test('detectFestival matches pre3Days pre-compensation window for Diwali 2026', () {
    ...
    test('Diwali pre3Days protocol reduces target calories by -150 kcal for buffer', () {
      final event = SmartFestivalNutritionEngine.festivalCalendar2026.firstWhere((e) => e.type == FestivalType.diwali);
    ...
    test('Diwali festivalDay protocol allocates +400 kcal & +15g protein for sweets', () {
      final event = SmartFestivalNutritionEngine.festivalCalendar2026.firstWhere((e) => e.type == FestivalType.diwali);
      final adjusted = engine.adjustTargets(
        baseline: baseline,
        event: event,
      ...
      expect(adjusted.satietyNudge, contains('Eat your high-protein sources'));
    });

    test('Diwali post1Day protocol allocates +1.0L water & 45-min recovery walk', () {
      final event = SmartFestivalNutritionEngine.festivalCalendar2026.firstWhere((e) => e.type == FestivalType.diwali);
      final adjusted = engine.adjustTargets(
        baseline: baseline,
        event: event,
    ...
    test('Holi festivalDay protocol allocates +1.0L water & +350 kcal buffer', () {
      final event = SmartFestivalNutritionEngine.festivalCalendar2026.firstWhere((e) => e.type == FestivalType.holi);
      final adjusted = engine.adjustTargets(
        baseline: baseline,
        event: event,
        relativeDay: FestivalDayRelative.festivalDay,
  ...
  group('FestivalNutritionNotifier Integration Tests', () {
    test('FestivalNutritionNotifier updates state reactively when checking festival dates', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);

      final notifier = container.read(festivalNutritionProvider.notifier);
      expect(container.read(festivalNutritionProvider).hasActiveAdaptation, isFalse);
