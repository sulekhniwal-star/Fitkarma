...
void main() {
  ...
  group('CoachEscalationService Triggers Tests', () {
    ...
    test(
      'shouldEscalate is true when plateau weeks >= 4 and adaptive calories adjusted',
      () {
        final state = const UserState(
          plateauWeeks: 4,
    ...
    test(
      'shouldEscalate is false when plateau weeks >= 4 but adaptive calories not adjusted',
      () {
        final state = const UserState(
          plateauWeeks: 4,
    ...
    test('shouldEscalate is true when consecutive relapse attempts >= 3', () {
      final state = const UserState(consecutiveRelapseAttempts: 3);
      expect(escalationService.shouldEscalate(state), isTrue);
    });

    test(
      'shouldEscalate is true when user explicitly requests human coach',
      () {
        final state = const UserState(userRequestedHumanCoach: true);
        expect(escalationService.shouldEscalate(state), isTrue);
      },
    );
  ...
  group('AiCoachChatNotifier Escalation Flow Tests', () {
    test(
      'keyword triggers automated escalation only for eliteCoach tier',
      () async {
        ...
        // Load conversation
        await notifier.loadCachedConversation(
          userId: 'user_1',
        ...
        expect(
          notifier.state.messages.last.messageContent,
          contains('Plan under human review.'),
        );
