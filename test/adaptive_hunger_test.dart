...
Widget _buildApp(ProviderContainer container) => UncontrolledProviderScope(
      ...
      child: const MaterialApp(
        home: Scaffold(body: HungerLoggingDialog()),
      ),
    );
...
void main() {
  group('AdaptiveHungerEngine Unit Tests', () {
    ...
    test('evaluateCravingRisk triggers 7 PM pre-emptive snacking alert on high-stress late-night sweet history', () {
      final historyLogs = [
        HungerCravingLog(
          id: '1',
    ...
    test('evaluateCravingRisk triggers high hunger warning when hungerScore == 5', () {
      final logs = [
        HungerCravingLog(
          id: '2',
    ...
    test('evaluateCravingRisk returns shouldTriggerNudge false on normal calm days', () {
      final ival = engine.evaluateCravingRisk(
        cravingLogs: [],
  ...
  group('HungerLoggingDialog UI & Controller Tests', () {
    ...
    testWidgets('renders HungerLoggingDialog with score selector, craving chips, and stress slider', (tester) async {
    ...
    testWidgets('selecting hunger score 4 and sweet craving logs to state', (tester) async {
      await tester.pumpWidget(_buildApp(container));
      await tester.pump();
