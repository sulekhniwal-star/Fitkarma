...
Widget _buildApp(ProviderContainer container) => UncontrolledProviderScope(
      ...
      child: const MaterialApp(
        home: Scaffold(body: SingleChildScrollView(child: MealQualityDisplayCard())),
      ),
    );
...
void main() {
  group('MultiDimensionalMealQualityEngine Unit Tests', () {
    ...
    test('600 kcal Rajma Rice + Curd + Salad (Tier 0, 28g Pro, 12g Fiber, Satiety 4.5) yields ~85/100 Grade S', () {
    ...
    test('600 kcal Fast Food Pizza (Tier 3, 18g Pro, 1.5g Fiber, Satiety 1.5) yields ~35/100 Grade C/D', () {
      final result = engine.calculateCompositeQualityScore(
        calories: 600,
        proteinG: 18,
        fiberG: 1.5,
      ...
      expect(result.breakdownSummary, contains('Ultra-processed food penalty'));
    });
  ...
  group('MealQualityDisplayCard UI & Controller Tests', () {
    ...
    testWidgets('renders MealQualityDisplayCard with 100-point gauge and grade badge', (tester) async {
      await tester.pumpWidget(_buildApp(container));
      await tester.pump();

      expect(find.byKey(const Key('meal_quality_display_card')), findsOneWidget);
      expect(find.byKey(const Key('meal_quality_score_text')), findsOneWidget);
      expect(find.byKey(const Key('meal_quality_grade_badge')), findsOneWidget);
      expect(find.text('Protein Density (2.5x)'), findsOneWidget);
    });

    testWidgets('adding logged foods updates meal quality score and grade badge', (tester) async {
      await tester.pumpWidget(_buildApp(container));
      await tester.pump();

      // Add high quality Rajma Chana Bowl
      container.read(foodProvider.notifier).addFood(
        const FoodItem(id: 'test_q1', name: 'Rajma Chana Bowl', calories: 400, protein: 25, carbs: 45, fat: 8, mealType: 'Lunch'),
      );
      await tester.pump();
