...
void main() {
  group('Hybrid Logical Clock (HLC) Tests', () {
    test('HLC comparison physical time precedence', () {
      ...
      final hlc1 = HLCTimestamp(physicalTime: t1, logicalCounter: 10, nodeId: 'nodeA');
      final hlc2 = HLCTimestamp(physicalTime: t2, logicalCounter: 0, nodeId: 'nodeA');
    ...
    test('HLC comparison logical counter tiebreaker', () {
      final t1 = DateTime(2026, 7, 12, 12, 0, 0);
      final hlc1 = HLCTimestamp(physicalTime: t1, logicalCounter: 5, nodeId: 'nodeA');
      final hlc2 = HLCTimestamp(physicalTime: t1, logicalCounter: 8, nodeId: 'nodeA');
    ...
    test('HLC comparison nodeId final tiebreaker', () {
      final t1 = DateTime(2026, 7, 12, 12, 0, 0);
      final hlc1 = HLCTimestamp(physicalTime: t1, logicalCounter: 5, nodeId: 'nodeA');
      final hlc2 = HLCTimestamp(physicalTime: t1, logicalCounter: 5, nodeId: 'nodeB');
  ...
  group('Sync Conflict Resolver Tests', () {
    ...
    test('Last-Write-Wins (LWW) with HLC', () {
      ...
      final local = MockEntity(
        hlc: HLCTimestamp(physicalTime: t2, logicalCounter: 0, nodeId: 'clientA'),
        data: 'Local newer state',
      );
      final remote = MockEntity(
        hlc: HLCTimestamp(physicalTime: t1, logicalCounter: 10, nodeId: 'clientB'),
    ...
    test('CumulativeLog merging cumulative values', () {
      ...
      final local = MockCumulativeLog(
        hlc: HLCTimestamp(physicalTime: t, logicalCounter: 0, nodeId: 'clientA'),
        syncBatchId: 'b1',
        count: 5,
      );
      final remote = MockCumulativeLog(
        hlc: HLCTimestamp(physicalTime: t, logicalCounter: 1, nodeId: 'clientB'),
        syncBatchId: 'b2',
        count: 10,
      );
