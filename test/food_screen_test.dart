...
void main() {
  Widget buildSubject() {
    return const ProviderScope(child: MaterialApp(home: FoodScreen()));
  }

  testWidgets(
    'FoodScreen shows low protein alert, logs items, updates macros, and hides alert when target met',
    (tester) async {
      ...
      expect(
        find.byKey(const Key('food_protein_alert_banner')),
        findsOneWidget,
      );
      expect(
        find.textContaining(
      ...
      final resultTile = find.byKey(
        const Key('food_search_result_Paneer Butter Masala'),
      );
      ...
      for (int i = 0; i < 3; i++) {
        ...
        final chickenResult = find.byKey(
          const Key('food_search_result_Chicken Tikka'),
        );
