...
void main() {
  ...
  group('GlycemicScoringEngine Unit Tests', () {
    ...
    test(
      'spike delta < 25 mg/dL yields 10.0 score (Optimal Energy Stability)',
      () {
        ...
        expect(
          eval.recommendation,
          contains('Great personal glycemic response'),
        );
      },
    );

    test(
      'spike delta 25-45 mg/dL yields 7.0 score (Moderate Glycemic Variance)',
      () {
        final eval = engine.computeFoodScore(
    ...
    test(
      'spike delta > 45 mg/dL yields 3.0 score & generates blunting recommendation',
      () {
        final eval = engine.computeFoodScore(
        ...
        expect(
          eval.recommendation,
          contains('High glucose spike detected (+53 mg/dL)'),
        );
  ...
  group('RetrospectiveGlycemicPipeline Unit Tests', () {
    ...
    test(
      'processMealHistory matches 90-min CGM readings and derives personal scores',
      () {
  ...
  group('glycemicScoringProvider Integration Tests', () {
    ...
    test(
      'glycemicScoringProvider computes evaluations from foodProvider reactively',
      () {
