...
void main() {
  ...
  Widget buildSubject() {
    return ProviderScope(
      ...
      child: const MaterialApp(home: AICoachScreen(userId: 'test_user')),
    );
  }

  testWidgets(
    'AICoachScreen renders header bento statistics and default instructions',
    (tester) async {
      ...
      expect(
        find.text('Ask me anything about your fitness, recovery, or diet!'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'AICoachScreen loads and renders cached chat messages from Drift',
    (tester) async {
      // Seed messages
      ...
      expect(
        find.text('Hello Arjun! How can I help you today?'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'Sending a user message optimistically inserts and triggers typewriter simulation',
    (tester) async {
      await tester.pumpWidget(buildSubject());
      await tester.pumpAndSettle();
      ...
      expect(
        find.textContaining('Your protein intake has averaged 58g'),
        findsOneWidget,
      );
      ...
      expect(
        cachedMsgs.any(
          (m) =>
              m.senderType == 'user' &&
              m.messageContent == 'Why am I plateauing?',
        ),
        true,
      );
      expect(
        cachedMsgs.any(
          (m) => m.senderType == 'ai' && m.messageContent.contains('protein'),
        ),
        true,
      );
    },
  ...
  testWidgets('Tapping suggested prompt chip sends message automatically', (
    tester,
  ) async {
    await tester.pumpWidget(buildSubject());
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'Toggling offline mode shows offline banner and handles offline error state',
    (tester) async {
      await tester.pumpWidget(buildSubject());
      await tester.pumpAndSettle();
      ...
      expect(
        find.text('Running in offline mode. Live AI responses will fail.'),
        findsNothing,
      );
      ...
      expect(
        find.textContaining('System Error: You are currently offline.'),
        findsOneWidget,
      );
    },
  );
