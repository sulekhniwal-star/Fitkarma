...
void main() {
  group('MicronutrientEngine Unit Tests', () {
    ...
    test('MicroRdaConfig calculates cohort-specific RDA targets', () {
      final maleConfig = MicroRdaConfig.forUser(
        isFemale: false,
        isVegetarian: false,
      );
      ...
      final vegFemaleConfig = MicroRdaConfig.forUser(
        isFemale: true,
        isVegetarian: true,
      );
    ...
    test(
      'evaluateDeficiencyRisk triggers B12 warning for low vegetarian B12 intake',
      () {
        ...
        expect(
          alerts.any((a) => a.affectedNutrient == MicronutrientType.vitaminB12),
          isTrue,
        );
    ...
    test(
      'evaluateDeficiencyRisk triggers Iron warning for low female iron intake',
      () {
        ...
        expect(
          alerts.any((a) => a.affectedNutrient == MicronutrientType.iron),
          isTrue,
        );
        expect(
          alerts.any((a) => a.recommendation.contains('Vitamin C')),
          isTrue,
        );
      },
    );
  ...
  group('MicronutrientDashboardScreen UI & Drift Integration Tests', () {
    ...
    testWidgets(
      'renders micronutrient dashboard with coverage gauge, alerts, and 8 biomarkers',
      (tester) async {
        ...
        expect(
          find.byKey(const Key('micro_overall_coverage_pct')),
          findsOneWidget,
        );
    ...
    testWidgets(
      'toggling demographics chip updates state and recalculates alerts',
      (tester) async {
        await tester.pumpWidget(_buildApp(container));
        await tester.pump();
    ...
    testWidgets(
      'adding logged foods to foodProvider updates micronutrient summary and persists to Drift',
      (tester) async {
        await tester.pumpWidget(_buildApp(container));
        await tester.pump();
