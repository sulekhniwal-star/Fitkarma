...
void main() {
  group('NutritionAdherenceEngine Unit Tests', () {
    ...
    test('100-Point Perfect Day calculates full scores for all 4 components', () {
      final now = DateTime.now();
      final records = [
        MealLogRecord(mealType: 'Breakfast', loggedAt: DateTime(now.year, now.month, now.day, 8, 30)),
        MealLogRecord(mealType: 'Lunch', loggedAt: DateTime(now.year, now.month, now.day, 13, 30)),
        MealLogRecord(mealType: 'Dinner', loggedAt: DateTime(now.year, now.month, now.day, 20, 30)),
      ];
    ...
    test('Calorie tolerance bounds award full pts within 10% and scale down to 0 at 25%', () {
    ...
    test('Logging completeness awards 20 pts for 3+ meals, 12 for 2 meals, 6 for 1 meal', () {
  ...
  group('adherenceProvider Integration Tests', () {
    test('adherenceProvider reactively computes adherence score & Karma award from foodProvider', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);

      final initialAdherence = container.read(adherenceProvider);
      expect(initialAdherence.breakdown.totalScore, greaterThanOrEqualTo(0.0));

      // Add high quality meals matching calorie & protein targets
      container.read(foodProvider.notifier).addFood(
        const FoodItem(id: 'add_1', name: 'Chicken Breast & Rice', calories: 600, protein: 50, carbs: 60, fat: 10, mealType: 'Lunch'),
      );
      container.read(foodProvider.notifier).addFood(
        const FoodItem(id: 'add_2', name: 'Paneer Bowl', calories: 500, protein: 40, carbs: 20, fat: 20, mealType: 'Dinner'),
      );
