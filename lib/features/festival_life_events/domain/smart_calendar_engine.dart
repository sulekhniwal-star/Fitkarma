...
class SmartCalendarEngine {
  ...
  SmartCalendarPlanReport generateSchedulePlan({
    ...
    final isHighBurnout =
        totalCognitiveLoadScore >= 60.0 || totalMeetingHours >= 5.5;

    // Recommended workout pacing based on cognitive fatigue
    final String workoutPacing;
    if (isHighBurnout) {
      workoutPacing =
          'Restorative Yoga, Breathwork & Light Mobility (कम तनाव पुनर्प्राप्ति योग व प्राणायाम)';
    } else if (totalCognitiveLoadScore >= 35.0 || totalMeetingHours >= 3.5) {
      workoutPacing =
          'Zone 2 Cardio, Functional Posture & Core Stability (मध्यम कार्डियो एवं कोर सुदृढ़ीकरण)';
    } else {
      workoutPacing =
          'High Intensity Strength & Hypertrophy Training (उच्च तीव्रता स्ट्रेंथ व वेट ट्रेनिंग)';
    }
    ...
    return SmartCalendarPlanReport(
      ...
      totalCognitiveLoadScore:
          double.parse(totalCognitiveLoadScore.toStringAsFixed(1)),
  ...
  List<SuggestedWellnessSlot> _identifyWellnessSlots({
    ...
    final dayStart =
        DateTime(targetDate.year, targetDate.month, targetDate.day, 6, 0);
    final dayEnd =
        DateTime(targetDate.year, targetDate.month, targetDate.day, 22, 0);
      ...
      if (eveningGap >= 15) {
        slots.add(
          SuggestedWellnessSlot(
            ...
            rationale:
                'Dissolves residual cortisol before dinner to protect deep REM sleep architecture.',
            regionalRationale:
                'गहरी नींद व तनाव मुक्ति हेतु शाम का विशेष विश्राम समय।',
            isOptimalTime: true,
          ),
        );
      }
  ...
  SuggestedWellnessSlot? _categorizeGapSlot({
    ...
    if (hour < 9 && gapMinutes >= 25) {
      return SuggestedWellnessSlot(
        ...
        rationale:
            'Elevates core body temperature and primes metabolic alertness before screen time.',
        regionalRationale:
            'दिन भर की ऊर्जा और एकाग्रता के लिए प्रातः सक्रियता।',
        isOptimalTime: true,
      );
    }
    ...
    if (hour >= 12 && hour <= 14 && gapMinutes >= 10) {
      return SuggestedWellnessSlot(
        ...
        rationale:
            'Blunts post-prandial glucose spike and prevents afternoon 2:30 PM sluggish brain fog.',
        regionalRationale:
            'भोजन के बाद रक्त शर्करा नियंत्रण और आलस्य दूर करने हेतु शतपावली।',
        isOptimalTime: true,
      );
    }

    // Buffer right before high-stress meeting
    if (nextEvent.type == CalendarEventType.highStressMeeting &&
        gapMinutes >= 5) {
      return SuggestedWellnessSlot(
        startTime: start,
        endTime: end,
        ...
        rationale:
            'Activates vagus nerve, stabilises heart rate variability, and sharpens cognitive clarity.',
        regionalRationale:
            'महत्वपूर्ण बैठक से पहले मानसिक शांति और स्पष्टता के लिए प्राणायाम।',
        isOptimalTime: true,
      );
    }
    ...
    if (gapMinutes >= 15) {
      return SuggestedWellnessSlot(
        ...
        rationale:
            'Releases thoracic tension from continuous laptop hunching and improves upper limb blood flow.',
        regionalRationale:
            'लैपटॉप पर निरंतर काम से गर्दन व पीठ के तनाव को दूर करने हेतु स्ट्रेच।',
        isOptimalTime: false,
      );
    }
  ...
  _BilingualTip _computeMealTimingTip(List<CalendarEventBlock> events) {
    if (events.isEmpty) {
      return const _BilingualTip(
        english:
            'Maintain balanced 3-meal cadence with steady hydration every 90 minutes.',
        hindi:
            'नियमित समय पर संतुलित आहार लें तथा प्रत्येक ९० मिनट में पर्याप्त जल पिएं।',
      );
    }
    ...
    final heavyMeeting = events.firstWhere(
      (e) =>
          e.type == CalendarEventType.highStressMeeting ||
    ...
    final meetingMinute =
        heavyMeeting.startTime.minute.toString().padLeft(2, '0');

    return _BilingualTip(
      english:
          'Heavy engagement scheduled at $meetingHour:$meetingMinute. Consume a light protein & complex carb snack (e.g., roasted chana + almond butter or buttermilk) 60 min before to eliminate lethargy.',
      hindi:
          '$meetingHour:$meetingMinute पर महत्वपूर्ण बैठक है। आलस्य से बचने हेतु १ घंटा पूर्व भुना चना, छाछ या हल्का प्रोटीन युक्त अल्पाहार लें।',
    );
  }
