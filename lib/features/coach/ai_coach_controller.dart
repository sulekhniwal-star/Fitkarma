...
class AiCoachChatState {
  ...
  AiCoachChatState copyWith({
    ...
    return AiCoachChatState(
      ...
      activeEscalationBriefing:
          activeEscalationBriefing ?? this.activeEscalationBriefing,
...
class AiCoachChatNotifier extends Notifier<AiCoachChatState> {
  ...
  Future<void> loadCachedConversation({
    ...
    final activeEscalation = escalations
  ...
  Future<void> sendMessage({
    ...
    if (state.subscriptionTier == 'eliteCoach') {
      ...
      } else if (lowerText.contains("relapse") ||
          lowerText.contains("distress")) {
        triggered = true;
        triggerReason = "Psychological distress signal in message: '$text'";
      } else if (lowerText.contains("human coach") ||
          lowerText.contains("human review") ||
          lowerText.contains("talk to a human")) {
        triggered = true;
        triggerReason =
            "User explicitly requested human coach in message: '$text'";
      }
    }
      ...
      state = state.copyWith(
        ...
        messages: [
          ...
          ChatMessage(
            ...
            senderType: 'ai',
            messageContent:
                "System Error: You are currently offline. Local cache loaded, but live coaching requires network connection.",
            createdAt: DateTime.now(),
          ),
        ],
      );
  ...
  Future<AzureCoachResponse> _callAzureCoachFunction(
    String message,
    String convId,
  ) async {
    ...
    String reply =
        "I analyzed your query: '$message'. Based on your profile and recovery metrics, everything looks solid. Keep up your active recovery routines.";
    List<String> sources = ["7-day logs"];

    if (message.toLowerCase().contains('plateau')) {
      reply =
          "Your weight has stabilized, but your muscle metrics are up. Your protein intake has averaged 58g for 6 days while your program requires 110g. Add paneer or 2 boiled eggs to breakfast to help your muscles recover.";
      sources = ["7-day logs", "user profile"];
    } else if (message.toLowerCase().contains('calorie')) {
      reply =
          "Since your sleep debt is -45m and you have mild quad soreness, your recovery capacity is moderate. I have adjusted your daily calorie intake target to 1,900 kcal (+100 kcal for repair).";
...
class AzureCoachResponse {
  AzureCoachResponse({required this.reply, required this.sources});
...
final aiCoachChatProvider =
    NotifierProvider<AiCoachChatNotifier, AiCoachChatState>(
