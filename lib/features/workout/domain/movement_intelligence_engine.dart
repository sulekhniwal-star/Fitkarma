...
enum FundamentalMovementPattern {
  horizontalPush(
      ...
      regionalName: 'क्षैतिज धक्का (चेस्ट प्रेस / दंड)'),
  horizontalPull(
      name: 'Horizontal Pull', regionalName: 'क्षैतिज खिंचाव (रोइंग / पीठ)'),
  verticalPush(
      name: 'Vertical Push', regionalName: 'ऊर्ध्वाधर धक्का (ओवरहेड प्रेस)'),
  verticalPull(
      name: 'Vertical Pull',
      regionalName: 'ऊर्ध्वाधर खिंचाव (पुल-अप्स / लैट्स)'),
  kneeDominant(
      name: 'Knee Dominant (Squat)',
      regionalName: 'घुटना प्रधान (स्क्वैट्स / देसी बैठक)'),
  hipDominant(
      name: 'Hip Dominant (Hinge)',
      regionalName: 'कूल्हा प्रधान (डेडलिफ्ट / हिंज)'),
  rotationalCore(
      name: 'Rotational & Core Integrity',
      regionalName: 'घूर्णन एवं कोर (मुद्गर / गदा घुमाना)');
...
class MovementIntelligenceEngine {
  ...
  static MovementBalanceReport evaluateMovementSymmetry({
    ...
    // Ratio Calculations
    final int pushSets =
        (patternSets[FundamentalMovementPattern.horizontalPush] ?? 0) +
            (patternSets[FundamentalMovementPattern.verticalPush] ?? 0);
    final int pullSets =
        (patternSets[FundamentalMovementPattern.horizontalPull] ?? 0) +
            (patternSets[FundamentalMovementPattern.verticalPull] ?? 0);

    final double pushPullRatio = pullSets > 0
        ? double.parse((pushSets / pullSets).toStringAsFixed(2))
        : 1.0;

    final int quadSets =
        patternSets[FundamentalMovementPattern.kneeDominant] ?? 0;
    final int hamSets =
        patternSets[FundamentalMovementPattern.hipDominant] ?? 0;

    final double quadHamRatio = hamSets > 0
        ? double.parse((quadSets / hamSets).toStringAsFixed(2))
        : 1.0;
    ...
    if (pushPullRatio > 1.3) {
      score -= 15.0;
      diagnoses.add(
          'Push-dominant bias detected ($pushPullRatio ratio). Excessive anterior deltoid and pec minor tension.');
      correctives.add(
          'Add 3-4 sets of Face Pulls or Prone Y-Raises to strengthen middle/lower trapezius.');
    } else if (pushPullRatio < 0.7) {
      score -= 10.0;
      diagnoses.add(
          'Pull-dominant bias ($pushPullRatio ratio). Chest volume below optimal hypertrophy threshold.');
      correctives.add('Add Incline DB Press or Desi Dand volume.');
    } else {
      diagnoses.add(
          'Optimal Push-to-Pull structural equilibrium ($pushPullRatio ratio). Protects glenohumeral joint integrity.');
    }

    if (quadHamRatio > 1.5) {
      score -= 15.0;
      diagnoses.add(
          'Quad-dominant knee bias ($quadHamRatio ratio). Hamstring-to-quad strength deficit increases ACL shear stress.');
      correctives.add(
          'Add Romanian Deadlifts (RDLs) or Nordic Hamstring Curls to balance posterior chain.');
    } else {
      diagnoses.add(
          'Balanced Lower Body Hinge-to-Squat ratio ($quadHamRatio). Protects patellar tendons and lumbar spine.');
    }

    final int rotationalSets =
        patternSets[FundamentalMovementPattern.rotationalCore] ?? 0;
    if (rotationalSets < 3) {
      score -= 10.0;
      diagnoses.add(
          'Low rotational and multi-planar core stimulus ($rotationalSets sets).');
      correctives.add(
          'Prescribe 10 minutes of Mudgar / Karlakattai 360 circular swings for rotational shoulder stability.');
    } else {
      correctives.add(
          'Maintain Mudgar & Desi Dand dynamic movement prep before loading heavy barbells.');
    }
    ...
    return MovementBalanceReport(
      ...
      structuralBalanceSummary:
          'Weekly movement distribution scored $qualityScore/100 on the Biomechanical Balance Index. '
  ...
  static MovementBalanceReport _buildEmptyReport() {
    return const MovementBalanceReport(
      ...
      patternWeeklySets: {},
      biomechanicalDiagnoses: [
        'Schedule workout sessions to evaluate your movement balance.'
      ],
      correctiveWarmupPrescriptions: [
        'Start every session with 5 minutes of joint mobility and Akhara Mudgar flow.'
      ],
