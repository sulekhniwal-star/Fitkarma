enum HungerType {
  biological(
      ...
      regionalName: 'शारीरिक भूख (ऊर्जा आवश्यकता)',
      isTrueHunger: true),
  hedonicCraving(
      name: 'Hedonic / Emotional Craving',
      regionalName: 'भावनात्मक तृष्णा / डोपामाइन लालसा',
      isTrueHunger: false),
  sleepDeprivedGhrelin(
      name: 'Sleep-Debt Ghrelin Surge',
      regionalName: 'नींद की कमी से प्रेरित भूख',
      isTrueHunger: false);
...
class HungerCravingsEngine {
  static const List<SatietyVolumeHack> indianVolumeHacks = [
    SatietyVolumeHack(
      ...
      calories: 45,
      satietyMechanism:
          '96% water volume distends gastric mechanoreceptors without caloric load.',
    ),
    SatietyVolumeHack(
      ...
      calories: 105,
      satietyMechanism:
          'Crunchy auditory sensory feedback satisfies chewing compulsion; provides 3g fiber.',
    ),
    SatietyVolumeHack(
      ...
      calories: 20,
      satietyMechanism:
          'Soluble mucilage expands into a viscous gel in the stomach, slowing gastric emptying.',
    ),
    SatietyVolumeHack(
      ...
      calories: 0,
      satietyMechanism:
          'Therapeutic warm liquid stimulates thermogenesis and oral satiety.',
    ),
  ];
  ...
  static HungerDiagnosticReport diagnoseHunger({
    ...
    if (sleepHoursLastNight < 6.0 && !willingToEatSimpleDalKhichdi) {
      return const HungerDiagnosticReport(
        ...
        rootCauseAnalysis:
            'Sleep deprivation (<6 hrs) elevates circulating ghrelin by +28% and suppresses leptin. Your brain is seeking fast glucose to compensate for central nervous fatigue.',
        recommendedVolumeHacks: indianVolumeHacks,
        instantActionStep:
            'Drink 400ml chilled water and take a 15-minute power nap or light stroll. Do not keep open snacks in sight.',
      );
    } else if (isSuddenOnset ||
        (isSpecificFoodCraved && !willingToEatSimpleDalKhichdi)) {
      return const HungerDiagnosticReport(
        detectedType: HungerType.hedonicCraving,
        rootCauseAnalysis:
            'Sudden, hyper-specific cravings for sweets, fried food, or bakery items are driven by dopamine loops and emotional stress (cortisol), not genuine biological energy depletion.',
        recommendedVolumeHacks: indianVolumeHacks,
        instantActionStep:
            'Activate the 15-minute craving delay protocol: Drink 400ml water and engage in an alternate task.',
      );
    } else {
      return const HungerDiagnosticReport(
        detectedType: HungerType.biological,
        rootCauseAnalysis:
            'Gradual onset and willingness to eat plain nutritious whole foods indicates true physiological glycogen and amino acid demand.',
        recommendedVolumeHacks: indianVolumeHacks,
        instantActionStep:
            'Consume a balanced high-protein meal: 30g protein (Paneer / Eggs / Daal) paired with complex carbs and fiber.',
      );
    }
  }
}
