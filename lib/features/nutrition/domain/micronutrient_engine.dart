enum MicronutrientType {
  vitaminB12(
      name: 'Vitamin B12',
      regionalName: 'विटामिन B12',
      unit: 'mcg',
      rda: 2.4,
      isCommonIndianDeficiency: true),
  vitaminD3(
      name: 'Vitamin D3',
  ...
  iron(
      name: 'Elemental Iron',
      regionalName: 'आयरन (लौह तत्व)',
      unit: 'mg',
      rda: 18.0,
      isCommonIndianDeficiency: true),
  calcium(
      name: 'Calcium',
      regionalName: 'कैल्शियम',
      unit: 'mg',
      rda: 1000.0,
      isCommonIndianDeficiency: false),
  magnesium(
      name: 'Magnesium',
      regionalName: 'मैग्नीशियम',
      unit: 'mg',
      rda: 400.0,
      isCommonIndianDeficiency: true),
  zinc(
...
class MicronutrientEngine {
  ...
  static MicronutrientReport evaluateMicronutrients({
    ...
    final statuses = [
      _createStatus(MicronutrientType.vitaminB12, b12,
          'Set Dahi (Curd), Paneer, Fortified Nutritional Yeast'),
      _createStatus(MicronutrientType.vitaminD3, d3,
          'Morning Sunlight (20 mins), Fortified Milk, Egg Yolks'),
      _createStatus(MicronutrientType.iron, iron,
          'Palak (Spinach), Roasted Chana, Sprouted Moong'),
      _createStatus(MicronutrientType.calcium, calcium,
          'Fresh Paneer, Set Dahi, Ragi (Finger Millet)'),
      _createStatus(MicronutrientType.magnesium, magnesium,
          'Pumpkin Seeds, Roasted Almonds, Brown Rice'),
      _createStatus(MicronutrientType.zinc, zinc,
          'White/Black Chana, Pumpkin Seeds, Cashews'),
    ];

    final avgPercentage =
        (statuses.fold<int>(0, (sum, s) => sum + s.percentageOfRda) /
                statuses.length)
            .round();
    ...
    if (isVegetarian) {
      warnings.add(
          'High B12 Deficiency Risk: Vegetarian diets lack active cobalamin. Consider a weekly 1000mcg methylcobalamin supplement.');
    }
    warnings.add(
        'Urban Sun Deprivation: 75%+ of Indians are deficient in Vitamin D3. Check serum 25(OH)D levels regularly.');
  ...
  static MicronutrientStatus _createStatus(
      MicronutrientType type, double intake, String topSource) {
