  ...
  static CircadianEnvironmentalReport evaluateDailyEnvironment({
    ...
    final milestones = [
      CircadianMilestone(
        ...
        guidance:
            'Get 10-15 mins of natural sunlight to anchor your master circadian clock (SCN).',
      ...
      CircadianMilestone(
        ...
        guidance:
            'Stop caffeine intake 8 hours prior to bedtime to ensure adenosine clearance.',
      ...
      CircadianMilestone(
        ...
        guidance:
            'Body temperature, grip strength, and reaction speed peak during this window.',
      ...
      CircadianMilestone(
        ...
        guidance:
            'Dim overhead bright lights and transition to amber lighting to stimulate melatonin.',
    ...
    if (hour >= 6 && hour < 11) {
      currentSummary =
          'Morning Cortisol Awakening: Perfect for outdoor hydration and light exposure.';
    } else if (hour >= 11 && hour < 16) {
      currentSummary =
          'Midday Metabolic Focus: Stay hydrated and schedule high-focus tasks.';
    } else if (hour >= 16 && hour < 20) {
      currentSummary =
          'Peak Neuromuscular Window: Prime time for resistance training and PRs.';
    } else {
      currentSummary =
          'Parasympathetic Wind-down: Lower room temperature and avoid blue light.';
    }

    // 4. Workout Timing & Environmental Safety Synthesis
    final String workoutRec;
    if (envSnapshot.aqiCategory == AqiCategory.veryPoor ||
        envSnapshot.aqiCategory == AqiCategory.severe) {
      workoutRec =
          'Outdoor air quality is Hazardous (AQI ${envSnapshot.aqi}). Strictly train indoors in filtered air.';
    } else if (envSnapshot.heatIndexC > 38.0) {
      workoutRec =
          'Severe Heat Index (${envSnapshot.heatIndexC.round()}°C). Shift intense training to early morning (6:30 AM) or air-conditioned gym.';
    } else {
      workoutRec =
          'Conditions are optimal for scheduled workout during your peak strength window (4:30 PM - 7:00 PM).';
    }
