...
class SubscriptionScreen extends ConsumerWidget {
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(tier.isPaid ? Icons.verified : Icons.account_circle, color: accentColor, size: 14),
              ...
              Container(
                ...
                child: const Text(
                  'Server Verified',
                  style: TextStyle(color: AppColors.karmaGreen, fontSize: 10, fontWeight: FontWeight.bold),
                ),
              ),
            ],
          ),
  ...
  Widget _buildBillingCycleSelector(SubscriptionState state, SubscriptionNotifier notifier) {
    return Container(
          ...
          return Expanded(
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? AppColors.surfaceElevatedHigh : Colors.transparent,
                  borderRadius: AppRadii.radiusSm,
                  border: isSelected ? Border.all(color: AppColors.focusBlue, width: 1) : null,
                ),
                child: Center(
                  child: Text(
                    cycle.name,
  ...
  Widget _buildTierCard(
    ...
    final priceStr = tier == SubscriptionTier.free
        ? '₹0 Free Forever'
        : (isAnnual ? '₹${tier.annualPriceInr}/year (₹${(tier.annualPriceInr / 12).round()}/mo)' : '₹${tier.monthlyPriceInr}/month');
    ...
    return GestureDetector(
      ...
      child: BentoCard(
        border: isSelected
            ...
            : (isCurrentActive ? Border.all(color: AppColors.karmaGreen, width: 1.5) : null),
        hasGlow: isSelected,
              ...
              children: [
                ...
                if (isCurrentActive)
                  ...
                  Container(
                    ...
                    child: const Text(
                      'MOST POPULAR',
                      style: TextStyle(color: AppColors.focusBlue, fontSize: 10, fontWeight: FontWeight.bold),
                    ),
                  )
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(
                  backgroundColor: isCurrentActive ? AppColors.surfaceElevated : accentColor,
                ...
                child: state.isLoading && isSelected
                    ...
                    : Text(
                        isCurrentActive
                            ...
                            : (tier == SubscriptionTier.free ? 'Downgrade to Free' : 'Select ${tier.name}'),
                        style: TextStyle(
                          color: isCurrentActive ? AppColors.textSecondary : (tier == SubscriptionTier.elite ? Colors.black : Colors.black),
                          fontWeight: FontWeight.bold,
                        ),
                      ),
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          ...EntitlementFeature.values.map((f) {
            ...
            final isUnlocked = _isFeatureUnlocked(state.entitlements.tier, requiredTier);
              ...
              child: Row(
                children: [
                  Icon(
                    ...
                    color: isUnlocked ? AppColors.karmaGreen : AppColors.textSecondary,
                    size: 16,
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                  ...
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                    decoration: BoxDecoration(
                      color: Color(requiredTier.accentColorValue).withValues(alpha: 0.12),
                      borderRadius: AppRadii.radiusSm,
                    ),
                    child: Text(
  ...
  bool _isFeatureUnlocked(SubscriptionTier userTier, SubscriptionTier requiredTier) {
    final int userLevel = userTier == SubscriptionTier.free ? 0 : (userTier == SubscriptionTier.pro ? 1 : 2);
    final int reqLevel = requiredTier == SubscriptionTier.free ? 0 : (requiredTier == SubscriptionTier.pro ? 1 : 2);
    ...
    return Column(
      children: [
        ...
        Text(
          ...
          style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary, fontSize: 9),
        ),
        const SizedBox(height: AppSpacing.sm),
        TextButton(
          onPressed: () => notifier.restorePurchases(),
          child: const Text('Restore Purchases', style: TextStyle(color: AppColors.focusBlue, fontSize: 11)),
        ),
      ],
    );
  }
