...
class DeepenedMetabolismScreen extends ConsumerWidget {
  ...
  Widget _buildHeroMetabolismCard(DeepenedMetabolismReport report) {
    ...
    final statusColor = isSuppressed ? AppColors.alertRed : AppColors.karmaGreen;
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      isSuppressed ? Icons.warning_amber_rounded : Icons.local_fire_department,
                      color: statusColor,
                      size: 14,
                    ),
              ...
              Container(
                ...
                child: Text(
                  ...
                  style: const TextStyle(color: AppColors.focusBlue, fontSize: 10, fontWeight: FontWeight.bold),
                ),
              ),
            ],
          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: AppTypography.bodySmall.copyWith(color: AppColors.energyOrange, fontSize: 10),
                    ),
                  ],
                ),
              ),
    ...
    return BentoCard(
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.pie_chart_outline, color: AppColors.focusBlue, size: 18),
              SizedBox(width: 6),
              Text(
                'Energy Expenditure Decomposition (TDEE)',
                style: TextStyle(color: AppColors.focusBlue, fontWeight: FontWeight.bold, fontSize: 12),
              ),
            ],
          ),
          const SizedBox(height: AppSpacing.sm),
          _buildDecompRow('BMR (Basal Metabolic Rate)', decomp.bmrCalories, 0.65, AppColors.karmaGreen),
          _buildDecompRow('NEAT (Non-Exercise Movement)', decomp.neatCalories, 0.18, AppColors.focusBlue),
          _buildDecompRow('EAT (Exercise Thermogenesis)', decomp.eatCalories, 0.12, AppColors.energyOrange),
          _buildDecompRow('TEF (Thermic Effect of Food)', decomp.tefCalories, 0.05, AppColors.gold),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(title, style: const TextStyle(color: AppColors.textPrimary, fontSize: 11)),
              Text('${kcal.round()} kcal (${(pct * 100).round()}%)', style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 2),
          ClipRRect(
  ...
  Widget _buildMacroCyclingCard(DeepenedMetabolismState state, DeepenedMetabolismNotifier notifier) {
    final cycling = state.report.macroCycling;
    final isTraining = state.dayType == ActiveDayType.trainingDay;
    final calories = isTraining ? cycling.trainingDayCalories : cycling.restDayCalories;
    final protein = isTraining ? cycling.trainingDayProteinGrams : cycling.restDayProteinGrams;
    final carbs = isTraining ? cycling.trainingDayCarbsGrams : cycling.restDayCarbsGrams;
    final fats = isTraining ? cycling.trainingDayFatsGrams : cycling.restDayFatsGrams;
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Row(
                children: [
                  ChoiceChip(
                    label: const Text('Training Day', style: TextStyle(fontSize: 10)),
                    selected: isTraining,
                    selectedColor: AppColors.karmaGreen,
                    backgroundColor: AppColors.surfaceElevated,
                    labelStyle: TextStyle(color: isTraining ? Colors.black : AppColors.textSecondary, fontWeight: FontWeight.bold),
                    onSelected: (_) => notifier.setDayType(ActiveDayType.trainingDay),
                  ),
                  const SizedBox(width: 4),
                  ChoiceChip(
                    label: const Text('Rest Day', style: TextStyle(fontSize: 10)),
                    selected: !isTraining,
                    selectedColor: AppColors.focusBlue,
                    backgroundColor: AppColors.surfaceElevated,
                    labelStyle: TextStyle(color: !isTraining ? Colors.white : AppColors.textSecondary, fontWeight: FontWeight.bold),
                    onSelected: (_) => notifier.setDayType(ActiveDayType.restDay),
                  ),
                ],
              ),
            ],
          ...
          Row(
            ...
            children: [
              ...
              _buildMacroPill('Protein', '${protein}g', '1.8g/kg', AppColors.focusBlue),
              _buildMacroPill('Carbs', '${carbs}g', 'Fuel', AppColors.karmaGreen),
              _buildMacroPill('Fats', '${fats}g', 'Hormones', AppColors.energyOrange),
            ],
          ),
        ],
      ),
  ...
  Widget _buildRefeedCard(DeepenedMetabolismState state, DeepenedMetabolismNotifier notifier) {
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.bolt, color: AppColors.energyOrange, size: 18),
                  const SizedBox(width: 6),
                  Text(
                    refeed.name,
                    style: const TextStyle(color: AppColors.energyOrange, fontWeight: FontWeight.bold, fontSize: 12),
                  ),
                ],
              ),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(
                  color: (isActive ? AppColors.karmaGreen : AppColors.energyOrange).withValues(alpha: 0.15),
                  borderRadius: AppRadii.radiusSm,
                ),
                child: Text(
                  isActive ? 'REFEED ACTIVE' : 'RECOMMENDED',
                  style: TextStyle(color: isActive ? AppColors.karmaGreen : AppColors.energyOrange, fontSize: 9, fontWeight: FontWeight.bold),
                ),
              ),
            ],
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: isActive ? AppColors.surfaceElevated : AppColors.energyOrange,
                shape: const RoundedRectangleBorder(borderRadius: AppRadii.radiusMd),
              ),
              onPressed: () => notifier.toggleRefeedMode(),
              child: Text(
                isActive ? 'End Refeed & Return to Deficit' : 'Activate +${refeed.extraCalories} kcal Refeed Window',
                style: TextStyle(color: isActive ? AppColors.textSecondary : Colors.black, fontWeight: FontWeight.bold, fontSize: 12),
              ),
            ),
          ),
        ],
    ...
    return BentoCard(
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.wb_sunny_outlined, color: AppColors.gold, size: 18),
              const SizedBox(width: 6),
              Text(
          ...
          Text(
            ...
            style: AppTypography.bodySmall.copyWith(color: AppColors.textPrimary, height: 1.35, fontSize: 11),
          ),
          const SizedBox(height: 2),
          Text(
            report.regionalCircadianAgniMealTip,
            style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary, height: 1.35, fontSize: 10),
          ),
        ],
      ),
    );
