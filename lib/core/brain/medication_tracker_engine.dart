...
class DrugInteractionEngine {
  ...
  List<InteractionWarning> checkSchedule(
    ...
    if ((medLower.contains('metformin') ||
            medLower.contains('glycomet') ||
            medLower.contains('glucophage')) &&
        currentMeal.carbsGrams > 80.0) {
      warnings.add(InteractionWarning(
        severity: InteractionSeverity.moderate,
        sourceMedication: med.medicationName,
        message:
            'Metformin combined with high simple carbs (>80g) can trigger rapid glucose swings and GI discomfort. Balance your plate with protein.',
      ));
    }

    // Case 2: Statins (Atorvastatin/Atorva/Lipitor) + Citrus / Grapefruit
    if ((medLower.contains('statin') ||
            medLower.contains('atorva') ||
            medLower.contains('lipitor')) &&
        currentMeal.foodItems.any((item) =>
            item.toLowerCase().contains('grapefruit') ||
            item.toLowerCase().contains('citrus'))) {
      warnings.add(InteractionWarning(
        severity: InteractionSeverity.high,
        sourceMedication: med.medicationName,
        message:
            'Grapefruit compounds inhibit metabolic enzymes, raising statin concentration in blood. Avoid grapefruit while on statins.',
      ));
    }

    // Case 3: Beta-blockers (Metoprolol/Ciplar/Propranolol) + High intensity exercise
    if ((medLower.contains('metoprolol') ||
            medLower.contains('ciplar') ||
            medLower.contains('propranolol')) &&
        proposedWorkout == WorkoutIntensityLevel.high) {
      warnings.add(InteractionWarning(
        severity: InteractionSeverity.moderate,
        sourceMedication: med.medicationName,
        message:
            'Beta-blockers blunt heart rate response. Do not rely on target HR zones today; use RPE (Rate of Perceived Exertion) to gauge intensity.',
      ));
    }
...
class RxNavInteractionService {
  ...
  List<InteractionWarning> fetchInteractionsOffline(
      List<MedicationSchedule> medications,
      MealSnapshot meal,
      WorkoutIntensityLevel workout) {
