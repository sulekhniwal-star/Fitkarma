...
class _Sha256Internal {
  ...
  static Uint8List hash(List<int> bytes) {
    ...
    for (var i = 0; i < padded.length; i += 64) {
      ...
      for (var j = 16; j < 64; j++) {
        final s0 = _rotr(words[j - 15], 7) ^ _rotr(words[j - 15], 18) ^ (words[j - 15] >>> 3);
        final s1 = _rotr(words[j - 2], 17) ^ _rotr(words[j - 2], 19) ^ (words[j - 2] >>> 10);
...
class WebhookSignatureVerifier {
  ...
  bool verifyRazorpaySignature({
    ...
    try {
      ...
      final computedSignature = hmac.map((b) => b.toRadixString(16).padLeft(2, '0')).join();
...
class CoachMatchingEngine {
  ...
  List<CreatorProfile> match({
    ...
    final scored = allCoaches.map((coach) {
      ...
      if ((client.goals.contains('weight_loss') || client.goals.contains('muscle_building')) &&
