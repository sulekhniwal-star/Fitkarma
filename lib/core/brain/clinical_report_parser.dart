...
class ClinicalReportParser {
  ...
  LabReportType identifyReportType(String text) {
    ...
    if (lower.contains('hemoglobin') || lower.contains('wbc') || lower.contains('platelet') || lower.contains('cbc')) {
      return LabReportType.cbc;
    } else if (lower.contains('cholesterol') || lower.contains('triglycerides') || lower.contains('ldl') || lower.contains('hdl')) {
      return LabReportType.lipidProfile;
    } else if (lower.contains('alt') || lower.contains('ast') || lower.contains('bilirubin') || lower.contains('lft')) {
      return LabReportType.lft;
    } else if (lower.contains('creatinine') || lower.contains('urea') || lower.contains('egfr') || lower.contains('kft')) {
    ...
    } else if (lower.contains('ferritin') || lower.contains('iron') || lower.contains('tibc')) {
  ...
  List<ExtractedBiomarker> extractValues(String text, LabReportType type) {
    ...
    if (type == LabReportType.cbc || text.toLowerCase().contains('hemoglobin')) {
      final hbMatch = RegExp(r'(?:hemoglobin|hb)\D*(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
      if (hbMatch != null) {
        ...
        biomarkers.add(ExtractedBiomarker(
          ...
          minReference: 13.5,
          maxReference: 17.5,
          status: val < 13.5 ? BiomarkerStatus.low : (val > 17.5 ? BiomarkerStatus.high : BiomarkerStatus.normal),
        ));
      }
    }

    if (type == LabReportType.lipidProfile || text.toLowerCase().contains('ldl')) {
      final ldlMatch = RegExp(r'ldl\D*(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
      if (ldlMatch != null) {
      ...
      final hdlMatch = RegExp(r'hdl\D*(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
      if (hdlMatch != null) {
  ...
  List<ClinicalInsight> generateInsights(List<ExtractedBiomarker> values, LabReportType type) {
    ...
    for (final b in values) {
      if (b.name == 'Hemoglobin' && b.status == BiomarkerStatus.low) {
        insights.add(const ClinicalInsight(
          ...
          description: 'Hemoglobin levels are below reference range (13.5–17.5 g/dL).',
          impactMessage: 'Impact: Lower energy levels, increased fatigue, reduced workout recovery capacity.',
          recommendedActions: [
      ...
      } else if (b.name == 'LDL Cholesterol' && b.status == BiomarkerStatus.high) {
        insights.add(const ClinicalInsight(
